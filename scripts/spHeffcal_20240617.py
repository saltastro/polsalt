...
def spHeffcal(DirList, fitcofnameList, **kwargs):
  ...
  # get polmapcor file, if specified, from data directory
#   if (polmapcor != 'None'):
...
#    cfitList = [cofnameList.index(name) for name in fitcofnameList]
  ...
  # obtain illumfiles
    ...
    illum_ftRC = np.zeros((files,targets,Rows,Cols),dtype=int)    
    for f,t in np.ndindex(files,targets):                   
        model_RC, modelarea, modelareanom = skypupillum(YX_dt[:,t], MBYX_df[:,f], "", rho_f[f])
        illum_ftRC[f,t] = ispup_RC.astype(float)[None,None,:,:]*model_RC
        
        if (debug & (f in (0,20,39,80,100,119)) & (t==12)): 
            np.savetxt("illumRC_"+str(f).zfill(2)+"_"+str(t)+".txt", illum_ftRC[f,t],fmt=" %2i")      
...
# python2.7 script.py spHeffcal.py 20230829/sci_blue5,20230829/sci_red3,20230830/sci_blue6,20230830/sci_red9 c0,cy1,cx1 calPA=91,90,91,90 debug=True
