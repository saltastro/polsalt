...
Lamp data: Fit HWCal data, evaluating FOV dependence of HW_Calibration (OBSOLETE, replaced by spHeffcal)
...
from polutils import datedfile, datedline, hwcal, viewstokes 
np.set_printoptions(threshold=np.nan) 

#---------------------------------------------------------------------------------------------
def sppolcal(fileList, **kwargs):
  ...
  # _v wavelength index of all files combined
  ...
  # _S (unnormalized) stokes: (I,Q,U)
    ...
    isdiffuse = (hdul[0].header.count('DIFFUSE') > 0)
    ishwcal = ((lampid == "NONE") | isdiffuse)
    iszeropt = (not ishwcal)
  
  # halfwave plate cal vs wav,FOV (1 file), +Track (files>1)
  # FOV: YX. normalized to +/- 25 mm  
  # Track MB mbyx. normalized to .04 m
    YXnorm = 25.
    mbnorm = 0.04      
    mbyx_df = np.zeros((2,files))
    targets_f = np.zeros(files,dtype=int)
    wav0_f = np.zeros(files)
    wavs_f = np.zeros(files,dtype=int)        
    ...
    for f,file in enumerate(fileList):
        ...
        wav0_f[f] = float(hdul['SCI'].header['CRVAL1'])
        wavs_f[f] = int(hdul['SCI'].header['NAXIS1'])        
                
    Targets = targets_f.max()
    waVs = int(((wav0_f + dwav*wavs_f).max() - wav0_f.min())/dwav)
    wav_v = np.arange(wav0_f.min(),wav0_f.min()+dwav*waVs,dwav)
        
    if iszeropt: 
        print "\nGrating spectropolarimetry on-sky zeropoint calibration"
        Wnorm = 3000.
        W0 = 6500.
        wav_W = W0*np.ones(1)
        Wavs = 1
    ...
    if ishwcal: 
        print "\nGrating spectropolarimetry polaroid efficiency calibration"
        wav_W = np.array(range(3200,3500,100)+range(3500,5000,50)+  \
            range(5000,7000,100)+range(7000,10200,200)).astype(float)
        Wavs = len(wav_W)              
        stokes_fpTW = np.zeros((files,2,Targets,Wavs))                                  
    ...
    stokes_fSTv = np.zeros((files,3,Targets,waVs))
    ok_fTv = np.zeros((files,Targets,waVs),dtype=bool)   
    for f,file in enumerate(fileList):
        hdul = pyfits.open(file)        
        
        print ("%2i %"+str(filelen)+"s %3i %8.3f %8.3f") % ((f, file, targets_f[f]) + tuple(mbyx_df[:,f])) 
        
        wav_w = np.arange(wav0_f[f],wav0_f[f]+dwav*wavs_f[f],dwav)                       
      ...
      # normalize FOV dims to edge
        ...
        if ishwcal:
            ...
            stokes_ptw = np.zeros((2,targets_f[f],wavs_f[f]))
            err_ptw = np.zeros((2,targets_f[f],wavs_f[f]))
        ...
        v0 = int((wav0_f[f] - wav_v[0])/dwav)
        stokes_fSTv[f,:,:targets_f[f],v0:(v0+wavs_f[f])] = stokes_Stw
        ok_fTv[f,:targets_f[f],v0:(v0+wavs_f[f])] = ok_tw
        
      # for hwcal, wavelength spline smooth onto grid for cal table
        if ishwcal:
  ...
  # do fit of calibration function
    ...
    dname_d = ['y','x']    
    if (files>1):
        mbpow_dc = [np.array([[0,1,2,0,0],[0,0,0,1,2]]),np.array([[0,1],[0,0]])][iszeropt]
    ...
    for W in range(Wavs):
        if (Wavs==1):
            ...
            if (ok_fT.sum() < files*Targets/2): continue                        
  ...
  # save smoothed data, residual rawstokes
    for f in outList:
        wav_w = np.arange(wav0_f[f],wav0_f[f]+dwav*wavs_f[f],dwav)
        v0 = int((wav0_f[f] - wav_v[0])/dwav)
        stokes_Stw = stokes_fSTv[f,:,:targets_f[f],v0:(v0+wavs_f[f])]
        targets = targets_f[f]
                
        np.savetxt(name+"_stokes_"+str(f)+"_tw.txt", np.vstack((np.tile(wav_w,2), \
            stokes_Stw.transpose((1,0,2)).reshape((targets,-1)))).T, fmt="%8.2f "+targets*"%9.4f ")
        if ishwcal:
            np.savetxt(name+"_stokes_"+str(f)+"_TW.txt",np.vstack((np.tile(wav_W,2),  \
        ...
        ok_tw = (okcal_tw & ok_fTv[f,:targets_f[f],v0:(v0+wavs_f[f])])
