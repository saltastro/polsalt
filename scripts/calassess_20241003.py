...
def calaccuracy_assess(infile, **kwargs):
  ...
  # compute culled fit to PA
    vstokes_vw, verr_vw = viewstokes(stokes_Sw,var_Sw,ok_w=ok_w)
    PAmean = np.median(vstokes_vw[1,ok_w])        
    vstokes_vw[1] = (vstokes_vw[1] - PAmean + 90.) % 180. + PAmean - 90.              
    pawt_w = np.zeros(wavs)
    pawt_w[ok_w] = 1./verr_vw[1,ok_w]
    pafit_d = np.polyfit(wav_w[ok_w],vstokes_vw[1,ok_w],2,w=pawt_w[ok_w]) 

  # cull outliers once
    pachi_w = pawt_w*(vstokes_vw[1] - np.polyval(pafit_d,wav_w))      
    dum,pamin,pamax,dum = fence(pachi_w)
    cull_w = ok_w & ((pachi_w < pamin) | (pachi_w > pamax))    
    okfit_w = ok_w & np.logical_not(cull_w) 
  ...
  # compute calibration mean PA ripple accuracy from stokes cycle mean, rotated to pq,pu 
    ...
    if debug:
        ...
        np.savetxt(name+"_assessdebug.txt",np.vstack((okfit_w,wav_w,dwav_w,pafit_w,stokes_Sw[0],vstokes_vw,verr_vw)).T,  \
            fmt=" %2i %6.1f %6.1f %8.3f  %10.3e %8.3f %8.3f  %8.3f %8.3f")
    ...
    if dofiles:
        np.savetxt(name+"_pafit.txt",np.vstack((wav_w,vstokes_vw[1],pafit_w,cull_w,okfit_w)).T,    \
            fmt=" %7.1f %9.4f %9.4f %2i %2i")     
