...
def spHeffcal(DirList, fitcofnameList, **kwargs):
    ...
    _p (0,1) output calibration file (P,PA, or Q,U)
    _S (unnormalized) stokes: input rawstokes (I,Q,U)
    _s (normalized) stokes: input rawstokes (q,u)
    ...
    usrcalcor = kwargs.pop('usrcalcor',"")     
...
#    cfitList = [cofnameList.index(name) for name in fitcofnameList]
  ...
  # obtain illumfiles
    ...
    illum_fstRC = np.zeros((files,2,targets,Rows,Cols),dtype=float)    
    for f,t in np.ndindex(files,targets): 
        wpangcor_s = np.zeros(2)
        if usrcalcor:
            wppat = obsDict['WPPATERN'][f].upper()
            patternList = open(datadir+'wppaterns.txt','r').readlines()
            for pattern in patternList:
                lineList = pattern.split()
                if (lineList[0] != wppat): continue                            
                wpangcor_s = float(usrcalcor)*np.array([lineList[3],lineList[5]]).astype(float)
                break
        for s in (0,1):                              
            model_RC = skypupillum(YX_dt[:,t], MBYX_df[:,f], "", rho_f[f]-wpangcor_s[s], imgtime_f[f],   \
                useshuttercor=useshuttercor, debug=debug)[0]
            illum_fstRC[f,s,t] = ispup_RC.astype(float)[None,None,:,:]*model_RC
        
            if (debug & (f in (0,20,39,80,100,119)) & (t==tcenter)): 
                np.savetxt("illumsRC_"+str(f).zfill(2)+"_"+str(s)+"_"+str(t)+".txt", illum_fstRC[f,s,t],fmt=" %6.3f")
        ...
        for t in np.where(okcomp_t)[0]:   
            ...
            for g in (0,1):
                ...
                for p in (0,1):               
                    ...
                    cof_C, covar_CC = curve_fit(make_PAPol(illum_fstRC[:,:,t],pupyx_dRC,pupoffyx_dt[:,t],cof0_c,cfitList), \
  ...
  # cull spectra to remove very low intensities
    ...
    rsslog.message('\n Fit deviation summary\n target file  %P Wavg %P Wstd   PA Wavg  PA Wstd', logfile)  
    ...
    if usrcalcor:
        qufac_ftw = (np.sin(2.*np.radians(vstokes_fptw[:,1])))**2     

    if debug:
        np.savetxt("qufac_fw.txt",np.vstack((wav_w,vstokes_fptw[:,1,tcenter],qufac_ftw[:,tcenter])).T,  \
            fmt = "%7.1f "+files*" %10.3f"+"  "+files*" %10.3f")
    ...
    for t in np.where(oktgt_t)[0]:    
        ...
        for p in (0,1):
            ...
            for w in np.where(ok_tw[t])[0]:
                ...
                qufac_f = []
                if usrcalcor: qufac_f = qufac_ftw[:,t,w]
                cof0_c = cof0_wc[w]                                          
                cof_C, covar_CC = curve_fit(make_PAPol(illum_fstRC[:,:,t],pupyx_dRC,pupoffyx_dt[:,t],cof0_c,   \
                    cfitList,dcofg_fc=dcofg_ftpc[:,t,p],dcal_f=p*dcalPA_f,qufac_f=qufac_f),            \
                    fidxList, vstokes_fptw[fidxList,p,t,w],p0=tuple(cof0_c[cfitList]))
                cof_c = np.copy(cof0_c)
                cof_c[cfitList] = cof_C
                err_c = np.zeros(cofs)
                err_c[cfitList] = np.sqrt(np.diagonal(covar_CC))
                fit_F =make_PAPol(illum_fstRC[:,:,t],pupyx_dRC,pupoffyx_dt[:,t],cof0_c,    \
                    cfitList,dcofg_fc=dcofg_ftpc[:,t,p],dcal_f=p*dcalPA_f,qufac_f=qufac_f) \
                    (fidxList,*tuple(cof_c[cfitList]))
                cof_tcpw[t,:,p,w] = cof_c
                fit_ftpw[ok_f,t,p,w] = fit_F
                if debug:
                    if (w != wmap): continue
                    PAPol_pRC[p], PAPol_pdRC[p] =make_PAPol_map(illum_fstRC[:,:,t],pupyx_dRC,pupoffyx_dt[:,t],cof0_c,    \
                        cfitList,dcofg_fc=dcofg_ftpc[:,t,p],dcal_f=p*dcalPA_f,qufac_f=qufac_f)  \
                        (fidxList,*tuple(cof_c[cfitList]))                
            for f in ffitList:
  ...
  # save cal cof file after scrunching _w down to cal _W 
    ...
    calzip.close()
    rsslog.message('\n Saving calibration file:  %s' % heffcalzip, logfile)
  ...
  # save et21 fit files
    ...
    rawzip.close()
    rsslog.message(' Saving PAPol raw file:  %s' % PAPolrawzip, logfile)    
    fitzip.close()
    rsslog.message(' Saving PAPol fit file:  %s' % PAPolrawzip, logfile)                                     
    return

#--------------------------------------
def make_PAPol(illum_fsRC,pupyx_dRC,pupoffyx_d,cof0_c,cfitList,dcofg_fc=[],dcal_f=[],qufac_f=[]):
    # cofnameList = ['c0','cy2','cx2','cxy','cr3','cy1','cx1','cy3','cx3']
    # illum_fsRC: illumination RC array, s=(0,1) for (q,u) wp pair
    # dcofg_fc: grating angle dependence correction for cof c
    # dcal_f: calibration polaroid PA correction
    # qufac_f: q - u linear factor (allq : 0, allu: 1) for illumination angle correction
    def fit(fList,*cof):
      ...
      # don't: pupoffyx_dRC = np.array([1.,-1.])[:,None,None]*pupoffyx_dRC              
        ...
        if len(qufac_f):
            PAPolfit_Fs = (PAPol_FRC[:,None,:,:]*illum_fsRC[fList]).sum(axis=(2,3))/    \
                illum_fsRC[fList].sum(axis=(2,3))
            PAPolfit_F = (1.-qufac_f[fList])*PAPolfit_Fs[:,0] + qufac_f[fList]*PAPolfit_Fs[:,1]
        else:
            PAPolfit_F = (PAPol_FRC*illum_fsRC[fList,0]).sum(axis=(1,2))/illum_fsRC[fList,0].sum(axis=(1,2)) 
...
# python2.7 script.py spHeffcal.py 20230829/sci_blue5,20230829/sci_red3,20230830/sci_blue6,20230830/sci_red9 c0,cy1,cx1 calPA=91,90,91,90 useshuttercor=True centeronly=True usrcalcor=-11.25 
