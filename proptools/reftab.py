...
refcolumnList=['name', 'catidx', 'targ_ra', 'targ_dec', 'width', 'len1', 'len2', 'tilt', 'mag', 'priority', 'fov_flag', 'inmask_flag']
refformatList=['%s',   '%4i',    '%10.5f',  '%10.5f',   '%6.1f', '%6.1f','%6.1f','%7.2f','%7.2f','%6.1f',   '%3i',       '%3i']
...
class RefTab:
    ...
    def clearrefstartable(self):
        ...
        inmask = np.where((self.slitlets.data['inmask_flag'] == 1)*(self.slitlets.data['itemtype'] == 'ref'))
...
#            for j in range(len(refcolumnList)):
#                item = self.parseItem('')
...
#                self.ui.tableWidgetRefStars.setItem(rows+1,j,item)
    ...
    def refchanged(self, x ,y):
        ...
        self.slitlets.updatevalue(ai, str(item.text()), refcolumnList[y])
    ...
    def addrefstarsfromcatalogue(self):
        ...
        * if slits have been added manually before adding from the catalogue
        ...
        flags
        '''
        refstars = np.where((self.slitlets.data['itemtype'] == 'ref') * (self.slitlets.data['fov_flag'] == 1))
        for i in refstars[0]:
...
#        self.slitlets.update_flags()
    ...
    def updaterefstartable(self):
        ...
        for i in inmask[0]:
            ...
            for j in range(len(refcolumnList)):
                item=QtGui.QTableWidgetItem(refformatList[j] % (self.slitlets.data[refcolumnList[j]][i]))            
