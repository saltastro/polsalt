        ...
        while dogroupcull:
          ...
          # shift all correctable slits: lenoverlap and crossmaxshift are in percent of length                
            ...
            if dogroupcull:                            
              ...
              # group is identified by after collidee that appears at least 3 times in both before and after
                i_dC = np.append(i1_dC,i0_dC,axis=1)                          
                i_I = np.unique(i_dC)
                nodes = len(i_I)
                IDict = dict(zip(i_I,range(nodes)))
                connect_II = np.zeros((nodes,nodes))
                for C in range(i_dC.shape[1]):
                    connect_II[IDict[i_dC[0,C]],IDict[i_dC[1,C]]] = 1
                connect_II += connect_II.T
                connect_graph = csr_matrix(connect_II)
                groups, group_I = connected_components(csgraph=connect_graph, directed=False, return_labels=True)
                dogroupcull = (groups > 0)
                
                if self.debug:
                    print "\npreshift colls: ", i0_dC
                    print "postshift colls: ", i1_dC
                    print "collidees: ", i_I
                    print "groups: ", group_I
                    
                if (not dogroupcull): continue
                                                 
                iList_g = np.empty(groups,dtype=object)
                maxcoll_g = np.zeros(groups)
                for g in range(groups):
                    iList_g[g] = list(i_I[np.where(group_I==g)[0]])             
                    maxcoll_g[g] = (np.abs(lenoverlap_i) - badoverlap_i)[iList_g[g]].clip(min=0).max()
                
                if self.debug:
                    print "groupiList: " 
                    for g in range(groups): print iList_g[g]
                    print "lenoverlap: "
                    for g in range(groups): print lenoverlap_i[iList_g[g]]
                    print "maxcoll_g: ", maxcoll_g
                                       
                dogroupcull = (maxcoll_g.max() > 0)
                if (not dogroupcull): continue
              # do one group at a time, worst first
                gcull = np.where(maxcoll_g == maxcoll_g.max())[0][0]
                 
                lenmean = [yp_i,xp_i][self.slitmask.impol][iList_g[gcull]].mean()
                cullpri = priority_i[iList_g[gcull]].min()
                
                culliArray = np.array(iList_g[gcull])[np.where(priority_i[iList_g[gcull]] == cullpri)[0]]           
                icull = culliArray[np.argmin(np.abs([yp_i,xp_i][self.slitmask.impol] - lenmean)[culliArray])]

                if self.debug:
                    print "\nculling slit from collision group:"
                    print "  group: ", iList_g[gcull]
                    print "  lenposns ",[yp_i,xp_i][self.slitmask.impol][iList_g[gcull]] 
