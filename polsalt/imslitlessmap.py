...
Locate targets in filter or slitless polarimetry image, save with 2D target map 
...
from polmaptools import sextract,catid,ccdcenter,gaincor,YXcalc
...
def imfilmap(infileList,mapTab,objectname,logfile='salt.log',debug=False):
  ...
  # run SeXtract
    ...
    if lampid == 'NONE':
        ...
        fwhmlo, fwhmhi = (0.6, 1.5)
    else:
        deblend = .1
        fwhmlo, fwhmhi = (0.3, 5.0)    
    sexTabcull,sexTab = sextract(medianimagefile,sigma=10.,deblend=deblend,minpix=minpix, \
        fwhmlo=fwhmlo,fwhmhi=fwhmhi,cull=True,logfile=logfile,debug=debug)  
  ...
  # sort stars (inversely) by flux, final list index _S
    ...
    Stars = s_S.shape[0]
    if debug:  
        rsslog.message('After crowding cull: '+str(Stars),logfile)
  ...
  # (2) cull candidates more than 3x fainter than faintest predicted target and fwhm < 1.35 median       
    ...
    if debug:
        rsslog.message(('After minimum flux and fwhm cull, O,E: %4i %4i' % (len(Slistp[0]),len(Slistp[1]))),logfile)
  ...
  # look over rot = +/-2 deg, y,x = +/-10 mm (by 0.1)
  # for lamp data, use much smaller the range, since telescope pointing is not involved

    rotrange = [1,2][int(lampid == 'NONE')]
    yxrange = [3,10][int(lampid == 'NONE')]
     
    cmatch = min(100,len(Slistp[0]))
    rot_R = 0.1*np.arange(-10*rotrange,10*rotrange)
    histmax_R = np.zeros(20*rotrange,dtype=int)
    maxidx_R = np.zeros(20*rotrange,dtype=int)

    offyx_Rdts = np.zeros((20*rotrange,2,targets,cmatch))
    for R,rot in enumerate(rot_R):
        ...
        offhist_yx,yedge,xedge = np.histogram2d(offyx_Rdts[R,0].flatten(),offyx_Rdts[R,1].flatten(),  \
            bins=np.arange(-yxrange,yxrange+0.1,0.1))            
        maxidx_R[R] = np.argmax(offhist_yx)
        maxyidx,maxxidx = np.unravel_index(maxidx_R[R],(20*yxrange,20*yxrange))
        histmax_R[R] = offhist_yx[(maxyidx-1):(maxyidx+2),(maxxidx-1):(maxxidx+2)].sum()
    Rmax = np.argmax(histmax_R)
    Rmax = int(np.median(np.where(histmax_R==histmax_R[Rmax])[0]))    
    yxoff_dR = (np.array(np.unravel_index(maxidx_R,(20*yxrange,20*yxrange))) - 10*yxrange)/10.    
    ...
    if debug:
        ...
        np.savetxt(name+"yx_d0t.txt",yx_dpt[:,0].T,fmt="%8.2f")
        np.savetxt(name+"yx_dS.txt",yx_dS[:,Slistp[0][:cmatch]].T,fmt="%8.2f")        
        np.savetxt(name+"yx_dS_offs.txt"    \
            ,(rotate2d(yx_dS[:,Slistp[0][:cmatch]],rotoff,center=yx0_dp[:,0])-yxoff_dp[:,0,None]).T,fmt="%8.2f")
        rsslog.message('\nRough IDs,  yoff xoff (mm), rot (deg), yxOEoff_d (mm):' ,logfile)
        rsslog.message(('   %3i '+5*'%8.2f ') % ((histmax_R.max(),)+tuple(yxoff_dp[:,0])+(rotoff,)+tuple(yxOEoff_d)),logfile)
  ...
  # note, the id'd targets are in the same order in Slistp[0] and [1], so S_i refers to both
    ...
    for iter in (1,2):
        offsettol = [1.,2.][int(lampid == 'NONE')]/iter
        t_i,S_i = catid(yxupdate_dpt[:,0], yx_dS[:,Slistp[0]],   \
            offsettol=offsettol, errtol=0.5/iter,debug=debug,logfile=logfile,name=name+'id'+str(iter)+'_')
      ...
      # (5) cull targets that are within target binradius of gap, fov edge, split image edge
        ...
        binrad = int(2.*max(fwhmedian,8./rcbin_d[1]))    # allow for 1 arcsec beamsplitter mosaic broadening
        ...
        if ((edgeculls>0) & debug):
            rsslog.message(('removing %2i targets too close to edges' % edgeculls),logfile)
        ...
        if debug:
            ...
            rsslog.message ("    Yoff   +/-     Xoff   +/-   rotoff  +/-   dyOEoff   +/-   dxOEoff   +/-  ",logfile)
            rsslog.message (5*" %7.3f %5.3f " % (dYX_d[0],dYXerr_d[0],dYX_d[1],dYXerr_d[1],   \
  ...
  # compute actual RA, DEC of opt axis for header
    ...
    rsslog.message((' %8.2f %8.2f %8.3f %8.3f %8.3f') %    \
  ...
  # split into separate O,E images (yxp), split at O,E separation point yxsep_d by integer row bins
    ...
    if debug:
        ...
        rsslog.message(('Found/cat magnification: %8.5f' % magrat),logfile)
  ...
  # tm files produced only for multi-file polarimetric observation
    ...
    for f,file in enumerate(infileList):
        ...
        rsslog.message(('Output file '+'t'+file+'. Background CRs: %4i' % bkgcrs_f[f]),logfile)
...
# ------------------------------------
def imspecslitlessmap(infileList, mapTab, objectname, calHdulList, cutwavoverride=0., logfile='salt.log',debug=False):
  # _i targets in mapTab catalog
  # _t id'd targets in tgtTab
  ...
  # get configuration data from first image
    ...
    isMOS = (hdul0[0].header['MASKTYP'].count('MOS') > 0)    
    imwav = 5000.                               # use for impol: close to center of spectrum

    cutwav = max(float(cutfilter[3:]),3200.,cutwavoverride)
    cutwav = cutwav - (cutwav % 100.)           # needs to line up with wav_W    
    name = objectname+"_"+str(int(cutwav))+"_imf_"
                       
  # compute usable FOV: slitless, most background is from red

    dum,rshift,dum,isfovblue_rc = RSSpolgeom(hdul0,cutwav)
    isfovred_rc = RSSpolgeom(hdul0,10200.)[3]
    if isMOS:
        isfov_rc = isfovred_rc | isfovblue_rc
    else:
        isfov_rc = isfovred_rc
    isfov_prc = shift(isfov_rc[:rows],(-rshift,0),order=0).reshape((2,prows,cols))    


    catid_i,ra_i,dec_i = mapTab['CATID'],mapTab['RA'],mapTab['DEC'] 
    catids = catid_i.shape[0]     

  # Get data from filtered cals _F.  Use _F with most targets: 
    calfilters = len(calHdulList)
    caltargets_F = np.zeros(calfilters,dtype=int)  
    boxwidth = 0
       
    for F in range(calfilters):
        caltargets_F[F] = len(Table.read(calHdulList[F]['TGT']))      
    useF = np.argmax(caltargets_F)
    tgtTab = Table.read(calHdulList[useF]['TGT'])        
    hdrF = calHdulList[useF][0].header                  
    wavl = float(hdrF['FILTER'][2:])
            
    rsslog.message(('Cal filter used: %6.0f' % wavl),logfile)         

    trkrhoF = hdrF['TRKRHO']       
    boxwidth = max(boxwidth, int(hdrF['BOXRC'].split()[0]))
    yxp0_dp = np.array([map(float,hdrF['YXAXISO'].split()),map(float,hdrF['YXAXISE'].split())]).T
    yxOEoff_d = -np.diff((yxp0_dp - RSSpolgeom(calHdulList[useF],wavl)[2]), axis=1)[:,0]   # OEbeam model correction                     
    ...
    yxcat_dpt = RSScolpolcam(YX_dt,wavl,coltem,camtem,yxOEoff_d=yxOEoff_d)
    yxpcat_dpt = yxcat_dpt - yx0_dp[:,:,None]
    catid_t = tgtTab['CATID']
    ...
    if debug:
        ...
        print >>geomfile, ('yx0_d  : %8.4f %8.4f'% tuple(yx0_d))
        print >>geomfile, ('yx0_dp    : '+4*'%8.4f ' % tuple(RSSpolgeom(calHdulList[useF],wavl)[0].flatten()))            
        print >>geomfile, ('yxp0_dp : '+4*'%8.4f ' % tuple(RSSpolgeom(calHdulList[useF],wavl)[2].flatten()))
        print >>geomfile, 'After OE update:'
        print >>geomfile, ('yxOEoff_d : '+2*'%8.4f ' % tuple(yxOEoff_d.flatten()))
        print >>geomfile, ('yx0_dp    : '+4*'%8.4f ' % tuple(yx0_dp.flatten()))
        print >>geomfile, ('yxp0_dp   : '+4*'%8.4f ' % tuple(yxp0_dp.flatten()))               
        
    ist_i = np.in1d(catid_i,catid_t)
    targets = ist_i.sum()
    yxp0_dp = RSSpolgeom(hdul0,imwav,yxOEoff_d)[2]      # reference: geometry at 5000 Ang plus mean OEbeam correction
    tel0_d = RAd0,DECd0,PAd0                            # reference: mean telescope position
    mag_t = tgtTab['MAG']
    ...
    if debug:
        ...
        print >>geomfile, ('yx0_dp : '+4*'%8.4f ' % tuple(yx0_dp.flatten()))    
        print >>geomfile, ('yxp0_dp: '+4*'%8.4f ' % tuple(yxp0_dp.flatten()))
        np.savetxt(objectname+'_dyxp_dpt.txt',np.vstack((np.where(ist_i)[0],dyxp_dpt.reshape((-1,targets)))).T, \
            fmt="%2i "+4*"%8.3f ")
  ...
  #   to establish target map for unfiltered imaging spectropolarimetry
    ...
    YX_dt = YXcalc(mapTab['RA'],mapTab['DEC'],RAd0,DECd0,PAd0,saltfps)[:,ist_i]
    ...
    yx_dptW = RSScolpolcam(YX_ds,wav_s,coltem,camtem,yxOEoff_d=yxOEoff_d).reshape((2,2,targets,Wavs))
    yxp_dptW = yx_dptW + dyxp_dpt[:,:,:,None] - yx0_dp[:,:,None,None]
    rcp_dptW  = rcpcenter_d[:,None,None,None] +     \
        (yxp_dptW - yxp0_dp[:,:,None,None])/binmm_d[:,None,None,None]              
    r0_pt = np.clip(np.ceil(rcp_dptW[0].min(axis=2)).astype(int),0,prows-1)
    r1_pt = np.clip(np.floor(rcp_dptW[0].max(axis=2)).astype(int),0,prows-1)    
    wavmap_ptr = np.zeros((2,targets,prows),dtype='float32')  
    cmap_ptr = np.zeros_like(wavmap_ptr)      
            
    for p,t in np.ndindex(2,targets):
        Rows = (r1_pt - r0_pt)[p,t]+1            
        dr_W = rcp_dptW[0,p,t] - r0_pt[p,t,None]                   
        wavmap_ptr[p,t,r0_pt[p,t]:r1_pt[p,t]+1] =   \
            interp1d(dr_W,wav_W,kind='cubic')(np.arange(Rows))                       
        cmap_ptr[p,t,r0_pt[p,t]:r1_pt[p,t]+1] =   \
            interp1d(dr_W,rcp_dptW[1,p,t],kind='cubic')(np.arange(Rows))

  # Form targetmap.  take wavelengths below color filter cutoff off targetmap
    ridx0_pt = np.argmax(wavmap_ptr > 0,axis=2)
    ridx1_pt = prows - np.argmax(wavmap_ptr[:,:,::-1] > 0,axis=2)-1
    Rows = (ridx1_pt - ridx0_pt).max() - 1 
  ...
  #  add 1/2 box on NIR side to use NIR cutoff for row guiding, document in TGT table
    ...
    grid_drc = np.indices((prows,cols))
    cidx_pt = np.zeros((2,targets),dtype=int)
    for p,t in np.ndindex(2,targets):
        cidx_pt[p,t] = int(np.round(cmap_ptr[p,t,ridx0_pt[p,t]:ridx1_pt[p,t]].mean()))
    ridx0_pt[1] -= Cols/2
    ridx1_pt[0] += Cols/2
    Rows += Cols/2
    targetmap_prc = np.zeros((2,prows,cols),dtype='uint8')
    ...
    for p,t in np.ndindex(2,targets):
        istargetpt_rc = (((gridprc_da[0] >= ridx0_pt[p,t]) & (gridprc_da[0] <= ridx1_pt[p,t])) &   \
                         (np.abs(gridprc_da[1] - cidx_pt[p,t]) <= Cols/2))
        isoverlap_prc[p] |= (istargetpt_rc & (targetmap_prc[p] > 0))
        targetmap_prc[p][istargetpt_rc & isfov_prc[p]] = t + 1 
        isbkg_prc[p] &= ~istargetpt_rc

  # gather reference data for prefTab            
    poskey_dp = np.array([['YO','YE'],['XO','XE']])
    rckey_dp = np.array([['R0O','R0E'],['C0O','C0E']])
    prefTab = mapTab[ist_i]
    prefTab['MAG'] = mag_t
    prefTab['Y'],prefTab['X'] = tuple(YX_dt)
    for p in (0,1):               
        prefTab[rckey_dp[0,p]],prefTab[rckey_dp[1,p]] = (ridx0_pt[p],cidx_pt[p]-Cols/2)     # O: too far to the left, E OK
    for p in (0,1):        
        prefTab[poskey_dp[0,p]],prefTab[poskey_dp[1,p]] = tuple(yxp_dptW[:,p,:,Wimref])
  ...
  # mark background CR's beyond extreme upper quartile fence as BP           
    ...
    DEC0 = Angle(DECd0, u.degree).to_string(unit=u.degree, sep=":")
    BOXRC = (("%3i %3i") % (Rows,Cols))
    CALIMG = ' '.join([calHdulList[F].filename().split('.')[0][-4:] for F in range(calfilters)])
    rsslog.message(('Blue wavelength cutoff: %6.0f' % cutwav),logfile)    
    rsslog.message(('Target box Rows,Cols: %3i %3i' % (Rows,Cols)),logfile)    
  ...
  # for each input fits, add targetmap extension, write to tm*.fits
    for f,file in enumerate(infileList):
        ...
        hdr['REFWAV'] = (imwav,"TGT ref wav (Ang)")      
        hdr['RA0'] = (RA0,"RA center")
        hdr['DEC0'] = (DEC0,"DEC center")
