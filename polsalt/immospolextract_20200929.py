...
from rssmaptools import ccdcenter,boxsmooth1d,RSScolpolcam,RSSpolgeom,impolguide,fence,fracmax
...
def immospolextract(fileList,name,logfile='salt.log',debug=False):
  ...
  # target position prediction from cal image
    ...
    WNIR = 9000.                                        # for debug evaluation of wavmap
    RNIRmap_tp = np.zeros((targets,2))
    for t,p in np.ndindex(targets,2):
        okmap_R = (wavmap_tpR[t,p] > 0.)
        rootList = UnivariateSpline(np.arange(Rows)[okmap_R],wavmap_tpR[t,p,okmap_R]-WNIR,s=0).roots()
...
#    dCmedian_ftp = np.ma.median(np.ma.masked_array(dC_ftpR,mask=~okprof_ftpR),axis=3).data
  ...
  # compute row-direction image motion relative to reference file using NIR half-point in optimal row exraction
    AA,BB,CC,DD,EE,FF = (-.262,12.407,.09784,.0525,-.1596,-.0236) # from zemax bs flexure, bsflex cal
  ...
  # median splitting change from ref at Y=0, giving relative beamsplitter tip dalfy_f
  
    drsplit0_ft = drsplit_ft/(1. + AA*YX_dt[0]/25.)
    
    drsplit_f = np.ma.median(np.ma.masked_array(drsplit_ft/(1. + AA*YX_dt[0]/25.),  \
        mask=~okdr_ft),axis=1).data 
    dalfy_f = BB*pixmm*rcbin_d[0]*drsplit_f
  # median overall shift from ref over targets, corrected for shift affect of bs tip 
    drmean_ft -= drsplit_f[:,None]*(CC + DD*YX_dt[0]/25.)[None,:]
    dr_f  = np.ma.median(np.ma.masked_array(drmean_ft,mask=~okdr_ft),axis=1).data
     
  # add in rho flexure of dalfy and dr from cal to ref from flexure models
    dalfycal = EE*(np.cos(np.radians(trkrhocal)) - 1.) + FF*np.sin(np.radians(trkrhocal))
    dalfyref = EE*(np.cos(np.radians(trkrho_f[fref])) - 1.) + FF*np.sin(np.radians(trkrho_f[fref]))
    dalfy_f += dalfyref - dalfycal       
    ur0ref,dum,dum = rssdtralign(dateobs,trkrho_f[fref])        
    dr_f += -(ur0ref - ur0cal) - CC*(dalfyref - dalfycal)/(BB*pixmm*rcbin_d[0])
  ...
  #   then add in dtr flexure correction to cal image by drflex shift (assumes guiding is small effect)
    ...
    for f in range(files):
        dalfyx_d = np.array([dalfy_f[f],0.])
        yx_dptW = RSScolpolcam(YX_ds,wav_s,coltem,camtem, dalfyx_d=dalfyx_d).reshape((2,2,targets,Wavs)) \
            + dyxp_dp[:,:,None,None]    
        wavmap_ftpR[f],dum,dum = specmap(yx_dptW,wav_W,ri_tpR,ci_tpC,dWdY_ptW,hdulcal, drflex=dr_f[f])
        for t,p in np.ndindex((targets,2)):
            okmap_R = (wavmap_ftpR[f,t,p] > 0.)        
            rootList = UnivariateSpline(np.arange(Rows)[okmap_R],wavmap_ftpR[f,t,p,okmap_R]-WNIR,s=0).roots()
    ...
    if debug:
        ...
        np.savetxt("dalfy_f.txt",np.vstack((np.arange(files),dalfy_f,dr_f,trkrho_f)).T, \
            fmt=" %3i %9.5f %8.3f %8.3f")        
  ...
  #   _w output spectrum (columns) binned at rbin  
    ...
    printstdlog     \
        ("\n        Output file               dY (\") dX (\") sigma (\") bsflx(deg) relflux crs", logfile)
  ...
  # save the result
    ...
    for f,file in enumerate(fileList):
        ...
        dver = dr_f[f]*1000.*pixmm*rcbin_d[0]/dtrfps                            
        dhor = dC_f[f]*1000.*pixmm*rcbin_d[1]/dtrfps
        sigma = sigma_f[f]*1000.*pixmm*rcbin_d[0]/dtrfps
        printstdlog ("%30s %8.3f %8.3f %8.3f %9.4f %8.3f %4i" %     \
            (outfile,dver,dhor,sigma,dalfy_f[f],relflux_f[f],crs_f[f]), logfile)
