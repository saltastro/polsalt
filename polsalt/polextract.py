...
def polextract(infileList, **kwargs):
    ...
    """
    logfile= kwargs.pop('logfile','polextract.log')    
    rsslog.history(logfile)
    rsslog.message(str(kwargs), logfile)        # for some reason, history is not printing kwargs    

    debug = (str(kwargs.pop('debug','False')).lower() == 'true')    # allow for string or boolean    
    fpidebug = kwargs.pop('fpidebug',(-1,-1,-1))
    if (type(fpidebug) is str): fpidebug = tuple([int(s) for s in fpidebug.split(',')])
        ...
        if grating=='N/A':              
          ...
          # longslit grating spectropolarimetry line map                     
            if ('WAVL' in mapTab.colnames):
                ...
                sppolextract(fileListj,name,logfile=logfile,debug=debug,fpidebug=fpidebug)
...
# ET21A blue 20201008
# cd /d/pfis/khn/20221008/sci
# python polsalt.py polextract.py tm*00[2-5].fits fpidebug=0,1,1 debug=True
# ET21B Moon red 20230801
# cd /d/pfis/khn/20230801/sci
# python polsalt.py polextract.py tm*010[2-5].fits fpidebug=2,0,15 debug=True
# ET21B Moon red 20230830
# cd /d/pfis/khn/20230830/sci_red
# python2.7 polsalt.py polextract.py tm*037[7-9].fits tm*0380.fits tm*0457.fits fpidebug=0,0,12 isdiffuse=True debug=True
