...
# fence(arr)
# chi2sample(f_ax,var_ax,ok_x,dx)
...
# ----------------------------------------------------------
def heffcal(heffcalfile,YX_dt,MByx_d,TRKyx_d,grang,trkrho,imgtime,wav_w, **kwargs):
  ...
  # interpolate polarization map coefficients in T for each t
  ...
  # NEXT: use interp2d  
    ...
    Rows,Cols = (skypupillum(YX_dT[:,T0], MByx_d, TRKyx_d, trkrho, imgtime))[0].shape
  ...
  # interpolate heffcal to desired wavelengths                
    ...
    return heffcal_ptw, ok_tw, modelarea_t, modelareanom_t, minillum_t
# ----------------------------------------------------------
...
def chi2sample(f_ax,var_ax,ok_x,X_x,dX):
...
#        print x0, X0, X0+dX_x[x0], len(xhArray), f_ax[0,xhArray[-1]]
#        exit()
        ...
        XmeanList.append(X_x[xhArray].mean().astype(int))
