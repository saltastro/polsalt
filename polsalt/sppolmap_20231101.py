...
def sparcpolmap(hdul,slitTab,objectname,sptilt,droff_pi,oksig_i,**kwargs):
  ...
  # create extraction targetmap, zeroing mutual collisions      
    targetmapstart_prc, r12start_dpic = targetmap(rce_pic,ok_pic,Rows_pi,rce_pic,ok_pic,Rows_pi,rows)

  # find and block O/E collisions
    targetmap_prc, r12_dpic =     \
        targetmap(rce_pic,ok_pic,Rows_pi,rce_pic[::-1],ok_pic[::-1],Rows_pi[::-1],rows)      
    targetmapOEcoll_prc = (targetmapstart_prc <> targetmap_prc)*targetmapstart_prc
    notbadspec_prc = (targetmapOEcoll_prc==0)                   # for bpm's of all tm files in this map    
...
def spmospolmap(dataList, objectname, calHdul, **kwargs):
  ...
  # input all data, split it into _pRc OE images for output       
    ...
    bkg_fpRc = np.zeros_like(image_fpRc)
    ...
    for f,hdul in enumerate(dataList):
      ...
      # reduce here for each file and put in BMAP
        ...
        if isdiffuse:
            ...
            bkgcrs_r = np.zeros(rows,dtype=int)
            csigmax_r = np.zeros(rows,dtype=int)
            imgerr_rc = np.zeros((rows,cols))             
            crsig_rc = np.zeros((rows,cols))                                    
            rknots_c = np.zeros(cols,dtype=int) 
            chkcr_c = np.zeros(cols,dtype=int)                                     
            istgt_rc = istarget_pirc.any(axis=(0,1))  
            okbkg_rc = okbin_rc & (~istgt_rc)
            crsiglim = 15.
            
          # find and flag cr's in rows, comparing with surrounding 4 columns
            for c in range(2,cols-2):
                imgerr_rc[:,c] =  image_rc[:,c] - np.median(image_rc[:,[c-2,c-1,c+1,c+2]],axis=1)
            
            for r in range(rows):        
                if (okbkg_rc[r,:].sum() == 0): continue
                cumbkg_c = np.cumsum(okbkg_rc[r])            
                chkcr_c[3:-2] = cumbkg_c[5:] - cumbkg_c[:-5]
                cList = list(np.where(chkcr_c == 5)[0])            # five good bkgs in a row                
                crsig_rc[r][cList] = imgerr_rc[r][cList]/np.sqrt(var_rc[r][cList])
                csigmax_r[r] = np.argmax(crsig_rc[r])                                                                    
                okbkg_rc[r] &= crsig_rc[r] < crsiglim
                bkgcrs_r[r] += (crsig_rc[r] >= crsiglim).sum()

            if debug:
                np.savetxt("bkgcrs_"+imgno+".txt",np.vstack((range(rows),bkgcrs_r,crsig_rc.max(axis=1), \
                    csigmax_r,image_rc[r,csigmax_r],var_rc[r,csigmax_r],imgerr_rc[r,csigmax_r])).T, \
                    fmt=" %4i %4i %8.2f  %4i %8.2f %8.2f %8.2f")
  
          # fit bkg in each column with Spline, with well-spaced knots at target edges inside data            
            for c in range(cols):
                if (okbkg_rc[:,c].sum() == 0): continue
                istgtarea_r = (istgt_rc[:,c] | np.logical_not(notbadspec_rc[:,c]))   # bkg is not in overlap
                rknot_n = (np.where(istgtarea_r[:-1] != istgtarea_r[1:])[0]).astype(float) + 0.5
                ...
                rdat_i = np.where(okbkg_rc[:,c])[0].astype(float)            
              ...
              # delete knots containing no background data and flag targets around it as bad                  
                ...
                istgt_n = istgt_rc[(rknot_n+0.5).astype(int),c]
                missingbkgArray = np.where((rdats_n==0) & (~istgt_n))[0][:-1]
                flagzerobkg_r = np.zeros(rows,dtype=bool)
                if len(missingbkgArray):                                          
                    for nmiss in missingbkgArray:
                        flagzerobkg_r |= ((range(rows) > rknot_n[nmiss-1]) & (range(rows) < rknot_n[nmiss+1]))
                    okbkg_rc[:,c] &= np.logical_not(flagzerobkg_r)                        
                    knotdeleteArray = np.concatenate((missingbkgArray,missingbkgArray+1))
                    rknot_n = np.delete(rknot_n, knotdeleteArray)
                    rdat_i = np.where(okbkg_rc[:,c])[0].astype(float)                    
                    rdats_n = np.append(np.diff(np.cumsum(okbkg_rc[:,c])[rknot_n.astype(int)]),0.)

              # delete outer knots, for UnivariateSpline routine
                rminbkg, rmaxbkg = (rdat_i[0], rdat_i[-1])
                isouterknot_n = (rknot_n <= rminbkg) | (rknot_n >= rmaxbkg)
                if (isouterknot_n.sum() > 0): 
                    rknot_n = np.delete(rknot_n, np.where(isouterknot_n)[0])
                rdats_n = np.append(np.diff(np.cumsum(okbkg_rc[:,c])[rknot_n.astype(int)]),0.)

              # short backgrounds put around single centered knot
                shortbkgList = np.where((rdats_n > 0) & (rdats_n < 8))[0]               
                for x,nshort in enumerate(shortbkgList):
                    rsum_n = np.append(np.diff(np.cumsum(np.arange(rows)*okbkg_rc[:,c])[rknot_n.astype(int)]),0.)
                    rknot_n[nshort-x] = rsum_n[nshort-x]/rdats_n[nshort-x] + 0.5 
                    rknot_n = np.delete(rknot_n,nshort-x+1)
                    rdats_n = np.append(np.diff(np.cumsum(okbkg_rc[:,c])[rknot_n.astype(int)]),0.)
                                        
                rknots_c[c] = rknot_n.shape[0]                    
                if (rknots_c[c] <2): continue
                ptsSW_n = SWcheck(rdat_i, rknot_n, k=1)                      

              # bail with extra info if Spline fpchec will give SW error
                checknot_N = np.concatenate(([rminbkg]*(2),rknot_n,[rmaxbkg]*(2))).astype(float)  
                if fpchec(rdat_i, checknot_N, 1):              
                    print c, fpchecpy(rdat_i, checknot_N, k=1)
                    print "  ", rknot_n                    
                    print "  ", ptsSW_n
                    np.savetxt("bkgfpchec453_c"+str(c)+".txt",np.vstack((range(rows),image_rc[:,c],   \
                    targetmap_prc[:,:,c],okbin_rc[:,c],istgt_rc[:,c],okbkg_rc[:,c])).T,fmt=" %4i %12.3f %4i %4i %2i %2i %2i")
                    exit()                    
                    continue

              # fit bkg for this column by spline; flag targetmap off for zero bkg                              
                bkg_rc[:,c] = LSQUnivariateSpline(rdat_i, image_rc[okbkg_rc[:,c],c], rknot_n, k=1)(range(rows))
                bkg_rc[:,c] *= (1. - flagzerobkg_r.astype(int))
                for p in (0,1):
                  targetmap_prc[p,flagzerobkg_r,c] = 0

            if debug:
                hdubkg = pyfits.PrimaryHDU(bkg_rc.astype('float32'))
                hdubkg = pyfits.HDUList([hdubkg])
                hdubkg.writeto("bkg453_rc.fits",overwrite=True)
                hdubkgsig = pyfits.PrimaryHDU(crsig_rc.astype('float32'))
                hdubkgsig = pyfits.HDUList([hdubkgsig])
                hdubkgsig.writeto("bkgsig453_rc.fits",overwrite=True)              
                                    
            for p in (0,1):
                bkg_fpRc[f,p] = shift(bkg_rc,(-rshift_p[p],0),order=0)[(p*prows):((p+1)*prows)]

        rsslog.message(('Input file '+infileList[f]+('  Bkg CRs: %3i' % bkgcrs_r.sum())),logfile)        
  ...
  # output with OE split calhdr data for target  
    ...
    for f,hdul in enumerate(dataList):   
        ...
        if isdiffuse:
            ...
            hdul.append(pyfits.ImageHDU(data=bkg_fpRc[f].astype('float32'),name='BMAP'))     
        hdul.append(pyfits.ImageHDU(data=wavmap_pRc,name='WMAP'))
...
#----------------------------
# cd ../20230830/sci_red
# python2.7 polsalt.py poltargetmap.py m*045[3-7].fits isdiffuse=True
