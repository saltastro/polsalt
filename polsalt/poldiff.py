 ...
 # get data and put on common wavelength/target grid, combining (commensurate) bins if necessary
    for b in range(obss):
        ...
        if dwav_b[b] == dWav:                        
            ...
            w1 = w_W[W1]                          
            stokes_bsTW[b,:,:,W0:W1] =        \
                stokesList_stw[b][:,t_T,w0:w1].reshape((-1,Targets,Wavs,subbins)).sum(axis=-1)
            var_bsTW[b,:,:,W0:W1] =           \
                varList_stw[b][:,t_T,w0:w1].reshape((-1,Targets,Wavs,subbins)).sum(axis=-1)
            cov_bsTW[b,:,:,W0:W1] =           \
                covList_stw[b][:,t_T,w0:w1].reshape((-1,Targets,Wavs,subbins)).sum(axis=-1)                                        
            ok_bsTW[b,:,:,W0:W1] =            \
                okList_stw[b][:,t_T,w0:w1].reshape((-1,Targets,Wavs,subbins)).all(axis=-1)
            werr_bTW[b,:,W0:W1] =            \
                werrList_tw[b][t_T,w0:w1].reshape((-1,Targets,Wavs,subbins)).all(axis=-1)                    
