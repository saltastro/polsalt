...
def sptilt(dataList,slitTab,debug=False):
  ...
  # get nominal spectrum location
    ...
    wav_pic,rce_pic,ok_pic = slitpredict(hdul,YX_di,wav_w,droff=0,sptilt=0,debug=False)    
...
def sparcpolmap(hdul,slitTab,objectname,sptilt,droff_pi,isdiffuse,badarc=2.,    \
  ...
  #   Compute FOV-average corrections
    ...
    if (okbrtfit_i.sum() >1):    
        ...
        noneokiArray = np.where(okbrtfit_pi.sum(axis=0)==0)[0]
        if len(noneokiArray):               
  ...
  # locate good id's, with cubic legendre fit wav vs col separately for O,E using full spectrum                       
    ...
    fiterrmean_pi = fiterr_pic.sum(axis=2)/(fiterr_pic>0).sum(axis=2)
  ...
  # zero out targetmap immediately around second order, if necessary
    ...
    if oksec_pic.sum(): 
        print "Second order possible in %i wavelengths" % oksec_pic.sum()  
