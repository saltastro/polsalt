...
def sparcpolmap(hdul,slitTab,objectname,sptilt,droff_pi,oksig_i,**kwargs):
  ...
  # predict spectra
    ...
    if debug:
        np.savetxt("wav_pic.txt",np.vstack((range(cols),wav_pic.reshape((-1,cols)))).T,fmt=" %5i "+2*entries*"%8.3f ")
        np.savetxt("rce_pic.txt",np.vstack((range(cols),rce_pic.reshape((-1,cols)))).T,fmt=" %5i "+2*entries*"%8.3f ")
        np.savetxt("ok_pic.txt",np.vstack((range(cols),ok_pic.reshape((-1,cols)).astype(int))).T,fmt=" %5i "+2*entries*"%2i ")
  ...
  # find and block O/E collisions
    ...
    okspec_prc = (targetmapOEcoll_prc==0)                   # for bpm's of all tm files in this map    
  ...
  # find and block second order contamination
    ...
    if oksec_pic.sum():
        ...
        if isseccoll:
            okspec_prc &= (targetmapseccoll_prc==0)
  ...
  # zero targetmap where fewer than 75% of Rows are now left
    ...
    for p,i in np.ndindex(2,entries):
        ...
        okspec_prc[p,isbadRows_rc] = False
    okspec_rc = okspec_prc.all(axis=0)
    if debug:            
  ...
  # allow for overlap of O and E beams
    ...
    okspec_prc = shift(okspec_rc[:rows],(-rshift,0),order=0).reshape((2,prows,cols))      
    for p in (0,1):   
    ...
    hdul.append(pyfits.ImageHDU(data=(~okspec_prc).astype('uint8'),name='BSM'))                  
...
def spmospolmap(dataList, objectname, calHdul, **kwargs):
  ...
  # output with calhdr data        
    for f,hdul in enumerate(dataList):   
        ...
        hdul['BPM'].data = (~(okbin_fprc[f] & okspec_prc)).astype('uint8')
