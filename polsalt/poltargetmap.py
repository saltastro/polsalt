...
from sppolmap import sptiltcalc, sparcpolmap, spmospolmap
   
datadir = os.path.dirname(__file__) + '/data/'
keywordfile = datadir+"obslog_config.json"
#np.set_printoptions(threshold=np.nan)

def poltargetmap(infileList, **kwargs):
...
#       linepolmap(objectlist_i,astromapo)
        ...
        # grating spectropolarimetry
            ...
            if (len(jarcList)>0):
                ...
                arcfilter = filterListj[jcal]
                arclamp = lampListj[jcal]                                
                isspec_j = np.in1d(np.arange(files),jarcList,invert=True)
            else:                                           
          # if no arc for this config look for a similar one with different config
                ...
                arcfilter = arcdata[0].header['FILTER']
                arclamp = arcdata[0].header['LAMPID'] 
          ...
          # evaluate spectral tilt, row offset from spectra, process arc, if not done already
            ...
            if os.path.exists(arcoutfile):
                ...
                tiltcor, droffset_pi, oksig_i = sptiltcalc(spdataList,slitTab,       \
                            cutwavoverride=cutwavoverride,debug=debug)           
                ...
                if ((~oksig_i).sum() > 0):
                    rsslog.message(("no signal found in targets: "+(~oksig_i).sum()*" %i")  \
                        % tuple(np.where(~oksig_i)[0]), logfile)                               
                rsslog.message("\n"+arcfile+" "+objectname+" "+arcfilter+" "+arclamp+" ARC", \
                    logfile)                        
                sparcpolmap(arcdata,slitTab,objectname,tiltcor,droffset_pi,oksig_i,pidebug=pidebug,   \
                    cutwavoverride=cutwavoverride,isdiffuse=isdiffuse,logfile=logfile,debug=debug)

          # process grating spectropolarimetry
            rsslog.message("\n"+objectname+"  "+np.array(filterListj)[isspec_j][0], logfile)
       
            spmospolmap(spdataList,objectname,arcdata,  \
                cutwavoverride=cutwavoverride,isdiffuse=isdiffuse,logfile=logfile,debug=debug)
