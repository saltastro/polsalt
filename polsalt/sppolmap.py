...
from polutils import rssdtralign,rssmodelwave,fargmax,fpchecpy,SWcheck,fence,legfit_cull
...
def sptiltcalc(dataList,slitTab,**kwargs):
  ...
  # get nominal spectrum location
    filteredgeadj = 25.  
    wmin = max(float(filter[3:])-filteredgeadj,cutwavoverride)
...
def sparcpolmap(hdul,slitTab,objectname,sptilt,droff_pi,oksig_i,**kwargs):
  ...
  # predict spectra
    filteredgeadj = 25.             # must be changed in 3 places in this file  
    wmin = max(float(filter[3:])-filteredgeadj,cutwavoverride)
    wav_w = np.arange(3000.,10000.,100.)
    wav_w = np.insert(wav_w[wav_w > wmin],0,wmin)   
    wavs = wav_w.shape[0]
    ...
    ok_pic = (ok_pic & oksig_i[None,:,None]) 
    
    if debug:
        np.savetxt("wav_pic.txt",np.vstack((range(cols),wav_pic.reshape((-1,cols)))).T, \
            fmt=" %5i "+2*entries*"%8.3f ")
        np.savetxt("rce_pic.txt",np.vstack((range(cols),rce_pic.reshape((-1,cols)))).T, \
            fmt=" %5i "+2*entries*"%8.3f ")
        np.savetxt("ok_pic.txt",np.vstack((range(cols),ok_pic.reshape((-1,cols)).astype(int))).T,   \
            fmt=" %5i "+2*entries*"%2i ")
  ...
  # for diffuse, assume slits centered on found spectra
    ...
    ishortArray = np.where(oksig_i & isshortslit_i)[0]      # _I is index in ishortArray    
    ...
    if isdiffuse:
        Rows_pi = oksig_i[None,:]*Rows*np.ones((2,entries),dtype=int)
    else:
  ...
  # for slits of same length, use same Rows_pi.  Add two bins on each side for safety
    lengthArray,idlength_I = np.unique(slitTab['LENGTH'][ishortArray],return_inverse=True)        
    for idlength in range(len(lengthArray)):
        IidArray = np.where(idlength_I == idlength)[0]
        iidArray = ishortArray[IidArray]        
  ...
  # adjust for mean wav error    
    if debug:
        ...
        np.savetxt("brtfit_"+arcimg+"_di.txt",  \
            np.vstack((range(entries),okbrtfit_i,brtfit_di,oewaverr_i,stdfit_i)).T,    \
  ...
  # get predicted spectrum dwav/dr for wavmap             
    ...
    okwav_pic = ((wav_pic > 0.) & (wav2_pic > 0.) & oksig_i[None,:,None])  # allow for endpoint rounding
    ok_pic &= okwav_pic
    wav_pic *= okwav_pic
    wav2_pic *= okwav_pic
  ...
  # use lstsq to get errors on Legendre coefficients
    ...
    for i in ishortArray:
        for p in (0,1):
            ...
            if debugname:
                np.savetxt(debugname+".txt",np.vstack((np.where(use_m),lampwav_m[use_m],legx_M, \
                    daterr_M,yerr_M,fiterr_M)).T,fmt = " %3i %8.2f %8.4f "+3*"%8.3f ")
  ...
  # create default extraction targetmap
    oknone_pic = np.zeros_like(okwav_pic)
    targetmapstart_prc, r12start_dpic = targetmap(rce_pic,okwav_pic,Rows_pi,rce_pic,oknone_pic,Rows_pi,rows)
        
  # now zero mutual collisions          
    targetmap_prc, r12mut_dpic = targetmap(rce_pic,okwav_pic,Rows_pi,rce_pic,okwav_pic,Rows_pi,rows)
    targetmapselfcoll_prc = (targetmapstart_prc <> targetmap_prc)*targetmapstart_prc
    r12_dpic = np.zeros_like(r12start_dpic)
    r12_dpic[0] = np.maximum(r12mut_dpic[0],r12start_dpic[0])
    r12_dpic[1] = np.minimum(r12mut_dpic[1],r12start_dpic[1])  
    
    if debug:        
        hdutmstart = pyfits.PrimaryHDU(targetmapstart_prc.astype('uint8'))
        hdutmstart = pyfits.HDUList([hdutmstart])
        hdutmstart.writeto("targetmap0_prc.fits",overwrite=True)  
        hdutmmutual = pyfits.PrimaryHDU(targetmap_prc.astype('uint8'))
        hdutmmutual = pyfits.HDUList([hdutmmutual])
        hdutmmutual.writeto("targetmap1_prc.fits",overwrite=True) 

  # find and block O/E collisions
    targetmap_prc, r12OE_dpic =     \
        targetmap(rce_pic,okwav_pic,Rows_pi,rce_pic[::-1],okwav_pic[::-1],Rows_pi[::-1],rows)      
    targetmapOEcoll_prc = (targetmapstart_prc <> targetmap_prc)*targetmapstart_prc
    notbadspec_prc = (targetmapOEcoll_prc==0)*(targetmapselfcoll_prc==0)   # for bpm's of all tm files in this map      
    targetmap_prc = targetmapstart_prc*notbadspec_prc
    r12_dpic[0] = np.maximum(r12OE_dpic[0],r12_dpic[0])
    r12_dpic[1] = np.minimum(r12OE_dpic[1],r12_dpic[1])         
    isOEcoll = ((targetmapOEcoll_prc > 0).sum() > 0)
    OEccollsList= [(targetmapOEcoll_prc==i+1).any(axis=(0,1)).sum() for i in range(entries)]
    
    if isOEcoll:
        rsslog.message(("Collisions found at wavelengths for targets:\n"+9*" "+entries*" %4i")   \
         % tuple(range(entries)),logfile)
        rsslog.message(("    O/E: "+entries*" %4i") % tuple(OEccollsList), logfile)    
        hduOEcoll = pyfits.PrimaryHDU(targetmapOEcoll_prc.astype('uint8'))
        hduOEcoll = pyfits.HDUList([hduOEcoll])
        hduOEcoll.writeto("OEcoll_prc.fits",overwrite=True) 
        hdutmOE = pyfits.PrimaryHDU(targetmap_prc.astype('uint8'))
        hdutmOE = pyfits.HDUList([hdutmOE])
        hdutmOE.writeto("targetmap2_prc.fits",overwrite=True)          
 
  # find and block second order contamination
    ...
    if oksec_pic.sum():
        targetmapsec_prc, r12sec_dpic =     \
            targetmap(rce_pic,okwav_pic,Rows_pi,rcesec_pic,oksec_pic,Rows_pi/3,rows)
        targetmapseccoll_prc = (targetmapstart_prc <> targetmapsec_prc)*targetmapstart_prc
        isseccoll = ((targetmapseccoll_prc > 0).sum() > 0)
        if isseccoll:
            notbadspec_prc &= (targetmapseccoll_prc==0)
            targetmap_prc[targetmapseccoll_prc > 0] = 0.
            r12_dpic[0] = np.maximum(r12sec_dpic[0],r12_dpic[0])
            r12_dpic[1] = np.minimum(r12sec_dpic[1],r12_dpic[1])        
            if (not isOEcoll):
                rsslog.message(("Collisions found at wavelengths for targets:\n"+9*" "+entries*" %4i")   \
                    % tuple(range(entries)),logfile)
            for p in (0,1):
                secccollsList= [(targetmapseccoll_prc[p]==i+1).any(axis=0).sum() for i in range(entries)]
                rsslog.message(("  %s sec: "+entries*" %4i")   \
                    % ((["O","E"][p],)+tuple(secccollsList)), logfile)
            
            second_prc = np.zeros((2,rows,cols),dtype=int)       # for debug fits         
            for p,i in np.ndindex(2,entries):
                rint_c = np.clip(np.round(rcesec_pic[p,i]).astype(int),0,rows-1).astype(int)
                second_prc[p,rint_c[oksec_pic[p,i]],np.arange(cols)[oksec_pic[p,i]]] = i+1 
            if debug:
                hdusecblk = pyfits.PrimaryHDU(targetmap_prc.astype('uint8'))
                hdusecblk = pyfits.HDUList([hdusecblk])
                hdusecblk.writeto("targetmap3_prc.fits",overwrite=True)             
                hdusspec = pyfits.PrimaryHDU(second_prc.astype('uint8'))
                hdusspec = pyfits.HDUList([hdusspec])
                hdusspec.writeto("second_prc.fits",overwrite=True) 
                hdusspeccoll = pyfits.PrimaryHDU(targetmapseccoll_prc.astype('uint8'))
                hdusspeccoll = pyfits.HDUList([hdusspeccoll])
                hdusspeccoll.writeto("speccoll_prc.fits",overwrite=True)                   
                hdusmap = pyfits.PrimaryHDU(targetmapsec_prc.astype('uint8'))
                hdusmap = pyfits.HDUList([hdusmap])
                hdusmap.writeto("targetmapsec_prc.fits",overwrite=True)                        
  ...
  # zero targetmap where fewer than 75% of Rows are now left
    ...
    if debug:            
        ...
        hdut0map.writeto("targetmap4_prc.fits",overwrite=True)       
    for p,i in np.ndindex(2,entries):
        isbadRows_rc = (isbadRows_pic[None,p,i,:] & (targetmap_prc[p] == i+1))        
        targetmap_prc[p,isbadRows_rc] = 0
        notbadspec_prc[p,isbadRows_rc] = False
    notbadspec_rc = notbadspec_prc.all(axis=0)
    if debug:            
        hdut1map = pyfits.PrimaryHDU(targetmap_prc.astype('uint8'))
        hdut1map = pyfits.HDUList([hdut1map])
        hdut1map.writeto("targetmap5_prc.fits",overwrite=True)
              
  # compute OE splitting rows for future splitting, allowing for overlap of O and E beams 
  ...
  # do gaincor, output arc hdu without OE split      
    ...
    hdul['SCI'].data = image_rc.astype('float32')
    hdul['VAR'].data = var_rc.astype('float32')
    hdul['BPM'].data = (~okimg_rc).astype('uint8')     
    ...
    hdul.append(pyfits.ImageHDU(data=(~notbadspec_rc).astype('uint8'),name='BSM'))
...
def spmospolmap(dataList, objectname, calHdul, **kwargs):
  # _i mask slits (entries in xml)
  # _f infileList index
  # _r image rows from calHdu
  # _R OE prows for output split target data
  ...
  # get configuration data from first image
    ...
    prows = rows/2                          # for OE split data
    ...
    notbadspec_rc = (calHdul['BSM'].data == 0)[:rows]          
    targetmap_prc = calHdul['TMAP'].data[:,:rows]
    wavmap_prc = calHdul['WMAP'].data[:,:rows]    
    tgtTab = Table.read(calHdul['TGT'])
    entries = len(tgtTab['CATID'])
    istarget_pirc = np.zeros((2,entries,rows,cols),dtype=bool)    
    for p,i in np.ndindex(2,entries):      
        istarget_pirc[p,i] = (targetmap_prc[p] == i+1)        
           
  # input all data, split it into _pRc OE images for output       
    ...
    targetmap_pRc = np.zeros((2,prows,cols),dtype=int)
    wavmap_pRc = np.zeros((2,prows,cols))  
    
  # for diffuse data, background is global detector/ image scattering: 
  # reduce here for each file and put in BMAP
    if isdiffuse:                    
        for f,hdul in enumerate(dataList):
            imgno = hdul.filename().split('.')[0][-4:]    
            gaincor_c = gaincor(hdul) 
            image_rc = hdul['SCI'].data[:rows]/gaincor_c[None,:]
            var_rc = hdul['VAR'].data[:rows]/gaincor_c[None,:]**2
            okbin_rc = ((hdul['BPM'].data[:rows]==0) & notbadspec_rc[:rows])                      
            bkg_rc = np.zeros((rows,cols))                                                                             
            istgt_rc = istarget_pirc.any(axis=(0,1))  
            okbkg_rc = okbin_rc & (~istgt_rc)
              
          # for each column interpolate in table of median bkg between inter-target knots, vs row midpt
          #   _T: possible separate backgrounds around 2*entries targets
            if (f==0):
              # map backgrounds using target map for first file
                rmidpt_Tc = np.zeros((2*entries+1,cols))
                isbkg_Trc = np.zeros((2*entries+1,rows,cols),dtype=bool)
                rknots_c = np.zeros(cols,dtype=int)
                flagzerobkg_rc = np.zeros((rows,cols),dtype=bool)                                          
                for c in range(cols):
                    if (okbkg_rc[:,c].sum() == 0): continue
                    istgtarea_r = (istgt_rc[:,c] | np.logical_not(notbadspec_rc[:,c]))   # bkg is not in overlap
                    rknot_n = (np.where(istgtarea_r[:-1] != istgtarea_r[1:])[0])
                    rmin,rmax = np.where(okbin_rc[:,c])[0][[0,-1]]
                
                    if (istgtarea_r[rmin]): rmin += np.where(~istgtarea_r[rmin:])[0][0]  # start and end outside tgt 
                    if (istgtarea_r[rmax]): rmax -= np.where(~istgtarea_r[rmax::-1])[0][0]                                                
                    rknot_n = rknot_n[np.where((rknot_n > rmin) & (rknot_n < rmax))[0]]                       
                    rknot_n = np.concatenate(([rmin],rknot_n,[rmax]))        

                 #  delete knots containing no background data and flag targets around it as bad                  
                    rdats_n = np.append(np.diff(np.cumsum(okbkg_rc[:,c])[rknot_n.astype(int)]),0.)
                    istgt_n = istgt_rc[(rknot_n+1),c]
                    missingbkgArray = np.where((rdats_n==0) & (~istgt_n))[0][:-1]                   
                    if len(missingbkgArray):                                          
                        for nmiss in missingbkgArray:
                            flagzerobkg_rc[:,c] = flagzerobkg_rc[:,c] |     \
                                ((range(rows) > rknot_n[nmiss-1]) & (range(rows) <= rknot_n[nmiss+1]))
                        okbkg_rc[:,c] &= np.logical_not(flagzerobkg_rc[:,c])                        
                        knotdeleteArray = np.concatenate((missingbkgArray,missingbkgArray+1))
                        rknot_n = np.delete(rknot_n, knotdeleteArray)                
                                        
                    rknots_c[c] = rknot_n.shape[0]
                    Tbkgs = rknots_c[c]/2                    
                    if (Tbkgs <1): continue                
                    rmidpt_Tc[:Tbkgs,c] = (rknot_n[::2] + rknot_n[1::2])/2.
                    
                    isbkg_Tr = (np.array([((range(rows) > rknot_n[2*T]) & (range(rows) <= rknot_n[2*T+1]))   \
                        for T in range(Tbkgs)]) & okbkg_rc[None,:,c])                    
                    isbkg_Trc[:Tbkgs,:,c] = isbkg_Tr

                rsslog.message('Backgrounds mapped ',logfile)

            for c in range(cols): 
              # calculate target backgrounds and interpolate bkg for this column
                Tbkgs = rknots_c[c]/2 
                if (Tbkgs <1): continue                                                                              
                bkgmed_T = np.ma.median(np.ma.masked_array(np.repeat(image_rc[:,c][None,:],Tbkgs,axis=0),   \
                    mask=~isbkg_Trc[:Tbkgs,:,c]),axis=1)                                                                                                                    
                bkg_rc[:,c] = np.interp(range(rows),rmidpt_Tc[:Tbkgs,c],bkgmed_T,left=0.,right=0.)

            bkg_rc[flagzerobkg_rc] = 0.                
            targetmap_prc[:,flagzerobkg_rc] = 0             

            image_pRc = np.zeros((2,prows,cols))
            var_pRc = np.zeros_like(image_pRc)
            okbin_pRc = np.zeros((2,prows,cols),dtype=bool)
            bkg_pRc = np.zeros_like(image_pRc)
                                                          
            for p in (0,1):
                image_pRc[p] = shift(image_rc,(-rshift_p[p],0),order=0)[(p*prows):((p+1)*prows)]
                var_pRc[p] = shift(var_rc,(-rshift_p[p],0),order=0)[(p*prows):((p+1)*prows)]       
                okbin_pRc[p] = shift(okbin_rc,(-rshift_p[p],0),order=0)[(p*prows):((p+1)*prows)]            
                bkg_pRc[p] = shift(bkg_rc,(-rshift_p[p],0),order=0)[(p*prows):((p+1)*prows)]    
                targetmap_pRc[p] = shift(targetmap_prc[p],(-rshift_p[p],0),order=0)[(p*prows):((p+1)*prows)]    
                wavmap_pRc[p] = shift(wavmap_prc[p],(-rshift_p[p],0),order=0)[(p*prows):((p+1)*prows)] 
            ...
            hdr['RSPLIT'] = calhdr['RSPLIT']        
            hdr['CALIMG'] = calhdr['CALIMG']                     
            hdul.append(pyfits.ImageHDU(data=targetmap_pRc.astype('uint8'),name='TMAP'))
            hdr['DIFFUSE'] = ("T","object fills slit")         
            hdul.append(pyfits.ImageHDU(data=bkg_pRc.astype('float32'),name='BMAP'))     
            hdul.append(pyfits.ImageHDU(data=wavmap_pRc.astype('float32'),name='WMAP'))
  ...
  #     flag bkg cols in TGT map    
        ...
        for t,p in np.ndindex(targets,2):
            ...
            rtgtArray = np.where((targetmap_prc[p]==i_t[t]+1).any(axis=1))[0]
            for r,c in np.array(np.meshgrid(rtgtArray,cbkgArray)).T.reshape((-1,2)):
                targetmap_prc[p,r,c] = 255
...
def targetmap(rce_pic,ok_pic,Rows_pi,rcecoll_pIc,okcoll_pIc,Rowscoll_pI,rows):
  ...
  # return targetmap, rowextent table r12_dpic (p=beams, nominally bottom,top) for each spectrum
  # do separately for beams.  Assume same number of targets in each
  # _I collision target
  
    beams,entries,cols = rce_pic.shape
    targetmap_prc = np.zeros((beams,rows,cols),dtype='uint8')
    r12_dpic = np.zeros((2,beams,entries,cols))
    r12coll_dpIc = np.zeros((2,beams,entries,cols))
    isselfcoll = (rce_pic == rcecoll_pIc).all()      
    mp_d = np.array([-1.,1.])

    r12_dpic = ok_pic.astype(float)*(rce_pic[None,:] +     \
        mp_d[:,None,None,None]*np.repeat(Rows_pi[:,:,None],cols,axis=2)[None,:]/2.)
        
    chkcoll_pIc = np.zeros_like(okcoll_pIc)         # count a collision with a spoiled spectrum
    for p,I in np.ndindex((beams,entries)):
        if (okcoll_pIc[p,I].sum()==0): continue
        c1,c2 = np.where(okcoll_pIc[p,I])[0][[0,-1]]          
        chkcoll_pIc[p,I] = ((np.arange(cols) >= c1) & (np.arange(cols) <= c2))
    r12coll_dpIc[:,chkcoll_pIc] = okcoll_pIc[chkcoll_pIc].astype(float)*(rcecoll_pIc[None,chkcoll_pIc] +    \
        mp_d[:,None]*np.repeat(Rowscoll_pI[:,:,None],cols,axis=2)[None,chkcoll_pIc]/2.)
            
    for p in range(beams):
...
def findlines(sci_c,var_c,ok_c,Lines=100,label="",logfile='salt.log'):
  ...
  #   Need at least +/-2 good pix around max   
    ...
    while (not ok_c[(cmax-2):(cmax+3)].all()):
        if label: 
            print >>rejectfile, -1, "badstart ", cmax, ok_c[(cmax-2):(cmax+3)].astype(int) ,    \
                sci_c[(cmax-2):(cmax+3)], var_c[(cmax-2):(cmax+3)]  
    ...
    while (L < (Lines-1)):
      ...
      # if new max is at edge of previous line or a gap, erase max only and continue          
        while (not ok1_c[(cmax-1):(cmax+2)].all()):
            if label: 
                print >>rejectfile, (" %2i " % (L+1)), "edge   ", cmax,     \
                    ok1_c[(cmax-1):(cmax+2)].astype(int)      
            ok1_c[cmax] = False
            cmax = np.argmax(sci_c*ok1_c)
      ...
      # if fit is inverted,colerr large,or puts max at least 1.5 pix away, skip to next line
        if ((quadfit_d[0] >= 0.) | ((quadfit_d[1]**2 - 4.*quadfit_d[0]*quadfit_d[2]) <= 0.) |   \
            ...
            if label: 
                print >>rejectfile, (" %2i " % (L+1)), "fit    ", cmax, dcmax, colerr,  \
                    np.where(use_c)[0],sci_c[use_c],quadfit_d
            continue
...
#----------------------------
# cd ../20230829/sci_blue3
# python2.7 polsalt.py poltargetmap.py mx*.fits isdiffuse=True
