...
from polutils import datedline, rssdtralign, fence, legfit_cull, fpchecpy
...
# ----------------------------------------------------------
def sppolextract(fileList,name,wbin='default',logfile='salt.log',debug=False,fpidebug=(-1,-1,-1)):
  ...
  # assemble target data from all files, compute background
    rsslog.message ("Assembling target data", logfile)
    ...
    for f,file in enumerate(fileList):
      ...
      # compute background
        if diffuse:
          # for diffuse, use bkg from targetmap
            bkg_fprc[f] = hdul['BMAP'].data
  ...
  # compute profiles for CRs and extraction from mean data, binned by profbin in column direction
    ...
    rsslog.message("  mean width (bins)/ spectrum column width/ profile 1000*fiterr:", logfile)    
    ...
    if debug:
        hdumean = pyfits.PrimaryHDU(signalmean_prc.astype('float32'))
        hdulmean = pyfits.HDUList([hdumean])
        hdulmean.writeto("signalmean_prc.fits",overwrite=True)     
  ...
  # fit 2d profile to spline(row), with row offset and scale fit by legendre
    ...
    profcwidth_pi = np.zeros((2,entries),dtype=int)    
    ...
    for p in (0,1):
        for i in range(entries):        
            ...
            fitprof_cR, okprof_cR, rfit_d, rrfit_d, profsamples_pi[p,i],fiterr0,fiterr,iters,funcalls =   \
                profile2dfit(signal_Rc.T,var_Rc.T,usesig_Rc.T,label,stackbin=profbin,snlim=snlim,debugname=debugname)

            profcwidth_pi[p,i] = np.diff(np.where(okprof_cR.any(axis=1))[0][[0,-1]])[0]
        ...
        rsslog.message((("    "+oktargs*"%4i ") % (tuple(profcwidth_pi[p,oktgt_i]))),logfile)
        rsslog.message((("    "+oktargs*"%4.0f ") % (tuple(1000.*fiterr_dpi[1,p,oktgt_i]))),logfile)                   
  ...
  # do extraction in row direction using fit profiles as weights
    ...
    for f in range(files):
        for p,i in np.ndindex(2,entries):
            ...
            oknorm_c = (xtrnorm_c > 0.)
            xtrwt_Rc = np.zeros((Rows,cols))
            xtrwt_Rc[:,oknorm_c] = (issig_Rc*sigprof_pirc[p,i,hasslit_pir[p,i]])[:,oknorm_c]/   \
                np.tile(xtrnorm_c[None,oknorm_c],(Rows,1))
            issig_c = (issig_Rc*xtrwt_Rc).any(axis=0) 
  ...
  # compute raw bin wavelength array of extraction, using profile center
    ...
    escapedprofList = []
    okwav_pic = ((rprof_pic>0) & isslit_pirc.any(axis=2))     
    for p,i in np.ndindex(2,entries):    
        if (~oktgt_i[i]): continue    
        for c in np.where(okwav_pic[p,i])[0]:
    ...
    if debug:
        np.savetxt("wav_"+img0+"_pic.txt",np.vstack((range(cols),wav_pic.reshape((-1,cols)))).T,    \
            fmt=" %4i "+2*entries*"%8.3f ")    
    
  # establish the wavelength range to be used
    if (wbin=='default'):
        grname_g = np.loadtxt(datadir+"gratings.txt",dtype=str,usecols=(0,))
        wbin_bg = np.loadtxt(datadir+"gratings.txt",usecols=(5,6,7,8),unpack=True)
        grnum = np.where(grname_g==grating)[0][0]
        if (rcbin_d[1] > 4):
            rsslog.message ("Nonstandard binning, set desired wbin as kwarg", logfile)
            exit()          
        wbin = wbin_bg[rcbin_d[1]-1,grnum]
...
# ----------------------------------------------------------
def profile2dfit(prof_ab,var_ab,ok_ab,label,stackbin=8,snlim=16.,debugname=None):
  ...
  # bin in the a (stacking) direction, initial cull based on sn of max deviation from linear fit
    ...
    okfit_i = np.zeros(stacks,dtype=bool)    
    for i in np.where(ok_ib.sum(axis=1) > 2)[0]:
        ...
        okfit_i[i] = (snmax_i[i] > snlim)
             
    samples_i = (okfit_i[:,None] & ok_iab.any(axis=2)).sum(axis=1) 
    ok_ib = (ok_ib & okfit_i[:,None])
    a_a = np.arange(profs)
    a_i = np.zeros(stacks)
    ok_ia = ok_iab.any(axis=2)                
    a_i[okfit_i] = (a_a[None,:]*ok_ia).sum(axis=1)[okfit_i]/ok_ia.sum(axis=1)[okfit_i]
  ...
  # for fnorm, b0 first guess, use 3 highest ok neighbor bins, fit quadratic                  
    ...
    okfit_i = (okfit_i & (ok_ib[i_i,idx0_i-1] & ok_ib[i_i,idx0_i+1]))
    fnorm_i = okfit_i.astype(float)*Fnorm_i
    ...
    okfit_i = (okfit_i & (b0_i > 1.) & (b0_i < bins-1))    
    b0_i[okfit_i] = -pcof1_i[okfit_i]/(2.*pcof0_i[okfit_i])

  # for width first guess use prof summed flux / fnorm
    width_i = np.zeros(stacks)
    width_i[okfit_i] = (ok_ib*prof_ib)[okfit_i].sum(axis=1)/fnorm_i[okfit_i]
    
  # cull out width outliers
    legx_i = 2*a_i/profs -1.
    okfitbefore_i = np.copy(okfit_i)      
    okfit_i = legfit_cull(legx_i,width_i,okfit_i,3,IQcull=2.)[1]
    samples_i = (okfit_i[:,None] & ok_iab.any(axis=2)).sum(axis=1)                 
    fnorm_i[okfit_i] = pcof2_i[okfit_i] - pcof1_i[okfit_i]**2/(4.*pcof0_i[okfit_i])        
    ok_ib = (ok_ib & okfit_i[:,None])

  # fit to Legendre polynomials.  Width is relative to center
    prof_ib[okfit_i] /= fnorm_i[okfit_i][:,None]
    blegcofs = 7
    bfit_d = np.polynomial.legendre.legfit(legx_i[okfit_i],b0_i[okfit_i],blegcofs-1)
    bbfit_d = np.polynomial.legendre.legfit(legx_i[okfit_i],width_i[okfit_i],2)
    ...
    if debugname:
        ...
        b00_di = np.zeros((2,stacks))
        for i in np.where(ok_ib.sum(axis=1)>0)[0]:
            b00_di[:,i] = np.where(ok_ib[i])[0][[0,-1]]
        np.savetxt("b0details_i_"+debugname+".txt",     \
            np.vstack((okfit_i,idx0_i,b00_di,pcof0_i,pcof1_i,pcof2_i,b0_i)).T, \
            fmt="%1i %4i %3i %3i %8.3f %9.0f %9.0f %8.3f")        
        np.savetxt("b0width_i_"+debugname+".txt",np.vstack((ok_ib.sum(axis=1),    \
            okfitbefore_i.astype(int),okfit_i.astype(int),Fnorm_i,snmax_i,legx_i,b0_i,width_i,b0err_i,widtherr_i)).T,    \
            fmt=3*" %2i"+" %9.1f "+6*" %8.3f ")

    bfit0 = bfit_d[0]       
    x0 = np.hstack((bfit_d[1:],bbfit_d))
    fiterr0 = profilefiterr(x0,legx_i,prof_ib,ok_ib,bfit0,label)
        
    xopt_d,fiterr,iters,funcalls,warnflag = fmin(profilefiterr,x0,xtol=0.5,ftol=0.01,maxiter=100,  \
        args=(legx_i,prof_ib,ok_ib,bfit0,label),full_output=True,disp=False)

    cofs = xopt_d.shape[0]            
    bfit_d = np.append(bfit0,xopt_d[:(cofs-3)])    
    bbfit_d = xopt_d[(cofs-3):]          

    profileSpline, B_ib, Bmax, Bmin = profilefit(xopt_d,legx_i,prof_ib,ok_ib,bfit0,label)[1:5]
  ...
  # output profile is interpolated across input fit problems      
    okfitprof_a = (ok_ia & okfit_i[:,None]).any(axis=0)    
    ...
    if debugname:
        ...
        print >> debugfile, ("bmax: %3i" % (ok_ab.shape[1]))        
        ...
        np.savetxt("b1width_i_"+debugname+".txt",np.vstack((ok_ib.sum(axis=1),    \
            okfit_i.astype(int),a_i,Fnorm_i,legx_i,b_i,width_i)).T,    \
            fmt=2*" %2i "+"%4i "+"%9.1f "+3*"%8.3f ")
    ...
    return outprof_ab, okoutprof_ab, bfit_d, bbfit_d, samples_i.sum(), fiterr0, fiterr, iters, funcalls

# ----------------------------------------------------------
def profilefiterr(xfit_d,legx_i,prof_ib,ok_ib,bfit0,label):
    return profilefit(xfit_d,legx_i,prof_ib,ok_ib,bfit0,label)[0]
    
# ----------------------------------------------------------
def profilefit(xfit_d,legx_i,prof_ib,ok_ib,bfit0,label):
    ...
    cofs = xfit_d.shape[0]    
    bins = ok_ib.shape[1]  
    bfit_d = np.append(bfit0,xfit_d[:(cofs-3)])    
...
#        print np.where(badknot_k)[0], Bknot_k[badknot_k]
    ...
    try:       
        ...
        print "fpchecpy: ", fpchecpy(Bsorted, Bknot_k[okknot_k], k=3)
...
#
# cd ../20230830/sci_blue5
# python2.7 polsalt.py polextract.py tm*.fits
