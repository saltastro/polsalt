...
import numpy as np
from numpy.polynomial import legendre as legfn
...
# ------------------------------------
def sptilt(dataList,slitTab,debug=False):
  ...
  # get nominal spectrum location
    wav_w = np.arange(float(filter[3:])-50.,10000.,100.)
    YX_di = np.array([slitTab['YCE'],slitTab['XCE']])
    entries = YX_di.shape[1]  
    wav_pic,rce_pic,ok_pic = slitpredict(hdul,YX_di,wav_w,articoff=0,droff_pi=0,sptilt=0,debug=False)
    rcidx_pic = np.round(rce_pic).astype(int)
    sptilt_pi = np.zeros((2,entries))
    drmax_pi = np.zeros((2,entries))
        
  # get mean row offset droff and tilt sptilt from row profile maximum left and right  
  #   Only use spectra within 16 rows of nominal   
    for p,i in np.ndindex(2,entries):
        ...
        drmax_pi[p,i] = (fargmax(imgright_R) + fargmax(imgleft_R))/2. - 15.               
        sptilt_pi[p,i] = (fargmax(imgright_R) - fargmax(imgleft_R))/(cright-cleft)
        if debug:        
            print >> debugfile,("%2i %3i %4i %4i %4i %7.1f %7.1f %8.4f %8.4f") %    \
                (p,i,c1,c2,cmid,cleft,cright,drmax_pi[p,i,],np.degrees(sptilt_pi[p,i]*rbin/cbin))
                
    droff = np.median(drmax_pi)             
    sptilt = np.degrees(sptilt_pi.mean()*rbin/cbin)     
    
  # redo individual spectrum offsets using droff and sptilt
  #   Only use spectra within 10x of maximum
    wav_pic,rce_pic,ok_pic = slitpredict(hdul,YX_di,wav_w,articoff=0,droff_pi=droff,sptilt=sptilt,debug=False)  
    rcidx_pic = np.round(rce_pic).astype(int)
    for p,i in np.ndindex(2,entries):
        c1 = np.where(wav_pic[p,i] > wav_w[0])[0][0]
        c2 = np.where((wav_pic[p,i] < wav_w[-1]) & (wav_pic[p,i] > 0.))[0][-1]    
        ridx_Rc = rcidx_pic[p,i][None,:] + np.arange(-15,16)[:,None]   
        img_R = imgsum_rc[ridx_Rc,range(cols)][:,c1:c2].sum(axis=1)    
        drmax_pi[p,i] = droff + fargmax(img_R) - 15.
        if debug:         
            print >>debugfile,("%2i %3i %4i %4i "+21*" "+"%8.4f ") % (p,i,c1,c2,drmax_pi[p,i,])

    if debug: debugfile.close()
     
    return sptilt, drmax_pi

# ------------------------------------

def sparcpolmap(hdul,slitTab,objectname,sptilt,droff_pi,isdiffuse,badarc=2.,    \
    logfile='salt.log',debug=False,pidebug=(-1,-1)):
    ...
    badarc: arc rejected if cubic leg cof > disp0*badarc
  ...
  # predict spectra
    wav_w = np.arange(float(filter[3:])-50.,9500.,100.)
    wavs = wav_w.shape[0]
    ...
    wav_pic,rce_pic,ok_pic = slitpredict(hdul,YX_di,wav_w,droff_pi=droff_pi,sptilt=sptilt,debug=False)
  ...
  # for diffuse, assume slits centered on found spectra
    ...
    shortslit_i = (slitTab['LENGTH'] < 20.)
    if (shortslit_i.sum()==0): exit()           # for now, punt if all longslit
    ishortArray = np.where(shortslit_i)[0]      # _I is index in ishortArray
    Icentershort = np.argmin((YX_di[:,shortslit_i]**2).sum(axis=0)) 
  ...
  # for slits of same length, use same Rows_pi.  Add two bins on each side for safety
    lengthArray,idlength_I = np.unique(slitTab['LENGTH'][shortslit_i],return_inverse=True)    
  ...
  # find up to 100 strongest lines
    ...
    for p in (0,1):
        for i in ishortArray:                          
            ...
            if ((not isdiffuse) & debug):
                print >>rflexfile, ((4*"%4i "+"%7.0f") % (p, i, rflex_pi[p,i], Rows_pi[p,i], rprofmax_pi[p,i])),
            
          # extract arc spectra
            for R in range(Rows_pi[p,i]):                            
                arc_c = img_rc[np.clip(rint_c+R-Rows_pi[p,i]/2,0,rows-1),range(cols)]                                           
                arc_pic[p,i] += arc_c
                var_pic[p,i] += var_rc[np.clip(rint_c+R-Rows_pi[p,i]/2,0,rows-1),range(cols)]
          ...
          # flag cr's
            cList = list(np.where(ok_pic[p,i])[0])
            chkcrcList = cList[1:-1]
            base_C = (arc_pic[p,i,cList[:-2]]+arc_pic[p,i,cList[2:]])/2.            
            spike_C = arc_pic[p,i,cList[1:-1]] - base_C
            iscr_C = (spike_C > base_C/2.) & (spike_C > 50.*np.sqrt(var_pic[p,i,cList[1:-1]]))
            iscr_c = np.zeros(cols,dtype=bool) 
            iscr_c[np.array(cList[1:-1])[iscr_C]] = True
            ok_pic[p,i] &= np.logical_not(iscr_c)      

            if ((not isdiffuse) & debug):
                print >>rflexfile, ("%3i" % iscr_c.sum()),
                if (((i==12) | (i==17)) & (p==1)):
                    np.savetxt("arc_"+str(i)+"_c.txt",np.vstack((arc_pic[:,i],var_pic[:,i],ok_pic[:,i],iscr_c)).T,    \
                        fmt=4*"%8.1f "+3*"%2i ")
  ...
  #   Compute FOV-average corrections
    ...
    corfit_dpi = np.zeros((2,2,entries))
    oewaverr_i = np.zeros(entries)
    okcorfit_pi = np.zeros((2,entries),dtype=bool)
          ...
          # identify by finding most frequent waverr = obs - lamp                                  
            ...
            if (i==ii):            
                ...
                np.savetxt("obswav_"+arcimg+"_"+str(p)+"_"+str(i)+"_L.txt",np.vstack((obswav_pil[p,i][okobsbrt_l],
                    obscol_pil[p,i][okobsbrt_l],isright_l[okobsbrt_l].astype(int))).T,fmt="%8.2f %8.2f %2i")
          # linear fit of waverr vs wavelength, using Legendre -1,1 scaling                                             
            okcorfit_pi[p,i] = (use_M.sum() > 7)           
            if (not okcorfit_pi[p,i]): continue
            legx_M = (lampwav_m[oklampbrt_m]-wav0)/(disp0*cols/2)
            corfit_dpi[:,p,i] = np.polyfit(legx_M[use_M],waverr_M[use_M],1)[0:2]
            stdfit_pi[p,i] = np.std(np.polyval(corfit_dpi[:,p,i],legx_M[use_M]) - waverr_M[use_M])
    ...
    okcorfit_i = okcorfit_pi.all(axis=0)       
    corfit_di = corfit_dpi.mean(axis=1)
    stdfit_i = np.sqrt((stdfit_pi**2).mean(axis=0))
    oewaverr_i = np.diff(corfit_dpi[1],axis=0)[0]         
    ...
    if debug:
        ...
        np.savetxt("corfit_dpi.txt",np.vstack((np.indices((2,entries)).reshape((2,-1)),  \
            okcorfit_pi.flatten(),corfit_dpi.reshape((2,-1)),stdfit_pi.flatten())).T,    \
            fmt=" %2i %3i %2i "+3*"%8.4f ")
        np.savetxt("corfit_di.txt",np.vstack((range(entries),okcorfit_i,corfit_di,stdfit_i)).T,    \
            fmt=" %2i %2i "+3*"%8.4f ")            

    (dwavoffdX, wavoff), cov_dd = np.polyfit(YX_di[1,okcorfit_i],corfit_di[1,okcorfit_i],1,cov=True)
    dwavoffdXerr = np.sqrt(np.diag(cov_dd))[0]
          
    articerr = np.degrees(cbin*pixmm*(wavoff/disp0)/FCam6000)
    dXmag = (dwavoffdX/disp0)*pixmm*cbin*FColl6000/FCam6000
    dXmagerr = (dwavoffdXerr/disp0)*pixmm*cbin*FColl6000/FCam6000

    yxwavoff = np.polyfit(YX_di[1,okcorfit_i],oewaverr_i[okcorfit_i],1)[1]
    yxOEoff_d = cbin*pixmm*np.array([0.,yxwavoff/disp0])   

    rsslog.message(("\narticulation adjustment (deg): %8.4f" % articerr),logfile)
    rsslog.message(("X magnification error (mm/mm): %9.5f +/- %9.5f" % (dXmag,dXmagerr)),logfile)
    if (np.abs(dXmag/dXmagerr) < 2.5):
        dXmag = 0.
        rsslog.message(("X magnification error set to zero"),logfile)   
    rsslog.message(("beamsplitter rotation (mm)   : %8.4f" % yxOEoff_d[1]),logfile)    

  # re-id with obswav corrected for artic error, Xmag error, and beamsplitter beam offset
  # adjust for mean wav error  
    wav_pic,rce1_pic,ok1_pic =    \
        slitpredict(hdul,YX_di,wav_w,articoff=-articerr,dXmag=dXmag,droff_pi=droff_pi,sptilt=sptilt,    \
        yxOEoff_d=yxOEoff_d,debug=False)
  ...
  # get predicted spectrum dwav/dr for wavmap             
    ...
    wav2_pic,rce2_pic,ok2_pic =    \
        slitpredict(hdul,YXoff_di,wav_w,articoff=-articerr,dXmag=dXmag,droff_pi=droff_pi,sptilt=sptilt, \
        yxOEoff_d=yxOEoff_d,debug=False)    
  ...
  # locate good id's, with cubic legendre fit wav vs col separately for O,E using full spectrum                       
    ...
    cullwt_pim = np.zeros_like(obscol_pim)
    oklampreject_im = np.zeros((entries,lamplines),dtype=bool)      # for lamp linelist optimization
    cofleg0_pid = corfit_pid[:,:,::-1]      # legfit returns leg cofs high to low
    cleg0_pi = np.zeros((2,entries))        # legfit center column is position of wav0
    culls_pi = np.zeros((2,entries),dtype=int)
    passes_pi = np.zeros((2,entries),dtype=int)
    ...
    for i in ishortArray:
        ...
        for p in (0,1):
            ...
            wtp_b = 1./colerr_pil[p,i,l_pb[p]]
            docull_b = (mbrtid_pil[p,i,l_pb[p]] == -1)
            debugname = ['',(str(p)+"_"+arcimg+("_%02i" % ii))][int(i==ii)]                                                                               
            corfit_pid[p,i],okfit_pb[p],fiterr_pb[p],stdfit_pi[p,i],culls_pi[p,i],passes_pi[p,i] =  \
                legfit_cull(legx_b,lampwav_m[m_b],ok_b,3,   \
                    wt=wtp_b,docull=docull_b,IQcull=2.,maxerr=2.*disp0,debugname=debugname)                  
                                                                                  
            disp_pb[p] = legfn.legval(legx_b+2./cols,cofleg0_pid[p,i]) - \
                legfn.legval(legx_b,cofleg0_pid[p,i])
            cullwt_pim[p,i,m_b[ok_b]] = wtp_b[ok_b]   
        ...
        waverr_pim[:,i,m_b[okfit_b]] = fiterr_pb[:,okfit_b]               
  ...
  # flag as bad all id'd lines that give systematically bad fit errors
    ...
    if debug:
        np.savetxt("cullwt_pim.txt",np.vstack((m1_M,lampwav_m[m1_M],    \
            cullwt_pim[:,:,m1_M].reshape((2*entries,-1)))).T,fmt=" %3i "+(2*entries+1)*"%8.3f ")    
        np.savetxt("legculls.txt",np.vstack((range(entries),culls_pi,passes_pi)).T,fmt=" %3i")   
    ...
    isfenced_M = ((meanerr_M < errfence_d[0]) | (meanerr_M > errfence_d[1]))
  ...
  # flag as bad at most 2 lines in a target with oe difference large compared to predicted error
    ...
    if debug:
        ...
        np.savetxt("fiterrp_0.txt", np.vstack((m1_M,lampwav_m[m1_M],waverr_pim[0,:,m1_M].T, \
            prederr_pim[0,:,m1_M].T)).T,fmt="%3i "+(2*entries+1)*"%8.3f ")
        np.savetxt("fiterrp_1.txt", np.vstack((m1_M,lampwav_m[m1_M],waverr_pim[1,:,m1_M].T, \
            prederr_pim[1,:,m1_M].T)).T,fmt="%3i "+(2*entries+1)*"%8.3f ")                                     
        np.savetxt("corfit_pid.txt",np.vstack((corfit_pid.transpose((0,2,1)).reshape((8,-1)),   \
            stdfit_pi)).T,fmt="%8.3f ")     
        ...
        isallcull_m = np.zeros(lamplines,dtype=bool)
        isallcull_m[m1_M[np.where(isfenced_M)[0]]] = True
        oeculls_m = oecull_im.sum(axis=0)
        oklamprejects_m = oklampreject_im.sum(axis=0) 
        np.savetxt("arccull_"+arcimg+".txt",np.vstack((range(lamplines),lampwav_m,oklamp_m,idlines_m, \
            isallcull_m,oeculls_m,oklamprejects_m)).T, fmt="%3i %8.2f %2i %3i %2i %3i %3i")
  ...
  # cull out targets with too few line id'd in arc
    minarclines = 8
    icullarcArray = np.where(idlines_i < minarclines)[0]
  ...
  # use lstsq to get errors on Legendre coefficients
    ...
    wavfiterr_pi = np.zeros((2,entries))     
    waverrmax_pi = np.zeros((2,entries))
    ...
    for i in ishortArray:
        for p in (0,1):
            ...
            a_Cd = np.vstack((np.ones(cols)[okpred_c], legx_pic[p,i,okpred_c],  \
                np.polyval(leg(2),legx_pic[p,i])[okpred_c],  \
                np.polyval(leg(3),legx_pic[p,i])[okpred_c])).T
            ...
            wt_M = 1./prederr_pim[p,i,use_m]
            a_Md = np.vstack((np.ones(idlines_i[i]), legx_M, np.polyval(leg(2),legx_M),  \
                np.polyval(leg(3),legx_M))).T * wt_M[:,None]
            b_M =  lampwav_m[use_m]*wt_M                     
            cofleg_pid[p,i],sumsqerr = la.lstsq(a_Md,b_M)[0:2]
            sigma = np.sqrt(sumsqerr/idlines_i[i])
            alpha_dd = (a_Md[:,:,None]*a_Md[:,None,:]).sum(axis=0)
            cov_dd = la.inv(alpha_dd)
            waverr_M = (lampwav_m[use_m] -     \
            ...
            wavchi2_pi[p,i] = ((waverr_M/prederr_pim[p,i,use_m])**2).mean()

            wav_rc = np.tile(np.polynomial.legendre.legval(legx_pic[p,i],cofleg_pid[p,i]),(rows,1))           
            isslit_rc = (targetmap_prc[p]==i+1)
            wavmap_prc[p,isslit_rc] =  (wav_rc +   \
                (np.arange(rows)[:,None] - rce1_pic[p,i][None,:])*dwavdr_pic[p,i][None,:])[isslit_rc]

            leg_dC = np.array([np.polyval(leg(d),legx_pic[p,i][isslit_rc.any(axis=0)]) for d in range(4)])
            var_C = np.array([(cov_dd*np.outer(leg_dC[:,C],leg_dC[:,C])).sum() for C in range(leg_dC.shape[1])])
            wavfiterr_C = sigma*np.sqrt(var_C)
            wavfiterr_pi[p,i] = wavfiterr_C.mean()
            waverrmax_pi[p,i] = wavfiterr_C.max()
    
            debugname = ['',(str(p)+"_"+arcimg+("_%02i" % ii))][int(i==ii)]                    
            if debugname:
                debugfile = open("legfinaldebug_"+debugname+".txt",'w') 
                for M,m in enumerate(np.where(use_m)[0]):
                    print >>debugfile, ((" %3i"+" %8.2f %8.4f %8.3f %8.3f") %    \
                        (m,lampwav_m[m],legx_M[M],waverr_M[M],prederr_pim[p,i,m])) 
                print >>debugfile
                wav_C = np.polynomial.legendre.legval(legx_pic[p,i],cofleg_pid[p,i])[isslit_rc.any(axis=0)]
                for CC in (range(0,len(wavfiterr_C),64)+[len(wavfiterr_C)-1,]):
                    print >>debugfile, (" %9.1f %10.4f" % (wav_C[CC], wavfiterr_C[CC]))

  # cull out targets with poorly defined nonlinear shape in arc 
    ibadarcArray = np.where((waverrmax_pi > badarc*disp0).any(axis=0))[0]
    
    for i in ibadarcArray:    
        rsslog.message(("Target %2i culled, O, E cof3err %6.2f %6.2f > %5.1f bins" %  \
            ((i,)+tuple(waverrmax_pi[:,i]/disp0)+(badarc,))),logfile)
        slitTab['CULL'][i] = "arc"                       
        ishortArray = np.delete(ishortArray,np.where(ishortArray==i)[0][0])
             
    rsslog.message("\nWavelength Fit errors(Ang), O, E: ",logfile)
    rsslog.message(("catidx "+2*"  rms   chi2   "+"lines"),logfile)
    for i in sorted(list(ishortArray)+list(icullarcArray)+list(ibadarcArray)):
        flag = ["","rejected"][(list(icullarcArray)+list(ibadarcArray)).count(i)]
        rsslog.message(("%4i  "+2*("%6.3f %6.1f  ")+"%4i"+" %s") %     \
            ((i,)+tuple(np.vstack((wavfiterr_pi[:,i],wavchi2_pi[:,i])).T.flatten())+(idlines_i[i],flag)),logfile)
    ...
    if debug:     
        ...
        np.savetxt("wavleg_pi_"+arcimg+".txt",np.vstack((np.indices((2,entries)).reshape((2,-1)),  \
            ...
            coflegpred_pid.reshape((2*entries,-1)).T,wavchi2_pi.flatten())).T, fmt="%3i %3i %4i "+9*"%8.3f ")              
        np.savetxt("waverr_piM.txt", np.vstack((m3_M,lampwav_m[m3_M],  \
            waverr_pim[:,:,m3_M].reshape((2*entries,-1)),colerr_pim[:,:,m3_M].reshape((2*entries,-1)))).T,    \
            fmt="%3i %8.3f "+4*entries*"%9.6f ")       
        ok_piM = (waverr_pim[:,:,m3_M] != 0)
  ...
  # for 300 l/mm, zero out targetmap immediately around second order
    if (grating == 'PG0300'):
        wavsec_pic,rcesec_pic,oksec_pic =     \
            slitpredict(hdul,YX_di,wav_w,order=2,droff_pi=droff_pi,sptilt=sptilt,debug=False)   
  ...
  # zero targetmap where fewer than 75% of Rows are now left
    ...
    if debug:            
        hdutmap = pyfits.PrimaryHDU(targetmap_prc.astype('uint8'))
        hdutmap = pyfits.HDUList([hdutmap])
        hdutmap.writeto("targetmap_prc.fits",overwrite=True)
...
# ------------------------------------
def spmospolmap(dataList, objectname, calHdul, isdiffuse, logfile='salt.log',debug=False):
  # _i mask slits (entries in xml)
  # _f infileList index
...
#----------------------------------------------
def slitpredict(hdul,YX_di,wav_w,order=1,articoff=0.,dXmag=0.,droff_pi=0.,sptilt=0.,yxOEoff_d=np.zeros(2),debug=False):
    hdr = hdul[0].header
    camtem = hdr['CAMTEM']
    coltem = hdr['COLTEM']
    dateobs =  hdr['DATE-OBS'].replace('-','')
    ...
    entries = YX_di.shape[1]
    wavs = wav_w.shape[0]
    if np.isscalar(droff_pi):
        droff_pi = np.repeat(droff_pi,2*entries).reshape((2,entries))        
    ...
    yx_dpwi = RSScolgratpolcam(YX_ds, wav_s, coltem, camtem, grating, grang, artic,       \
        dateobs, order=order, yxOEoff_d=yxOEoff_d, debug=debug).reshape((2,2,wavs,-1))
    ...
    for p,i in np.ndindex(2,entries):
        fcw = interp1d(rc_dpwi[1,p,:,i],wav_w,bounds_error=False,      \
        ...
        wav_pic[p,i] *= ((wav_pic[p,i] > wav_w[0]) & (wav_pic[p,i] < wav_w[-1]))
...
def lineid(calhdr,lampwav_m,oklamp_m,okline_pl,mbrtid_pl,obscol_pl,fmax_pl,wav_pc,logfile='salt.log',debugname=''):
  ...
  # cull by fmax E/O       
    ...
    fmaxfit_d,ok_b,fiterr_b,stdfit,culls,passes =  \
        legfit_cull(legx_b,fmaxOE_b,ok_b,fitorder,IQcull=3.)
    ok3_b = np.copy(ok_b)
  ...
  # restore bright-line id's, if necessary (must be ok in both O and E)
    ...
    if debugname:
        ...
        print >>debugfile, (("fmaxOEcull: "+culls*"%3i \n" % tuple(mcullArray)))
