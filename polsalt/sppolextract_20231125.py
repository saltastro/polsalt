  ...
  # fit 2d profile to spline(row), with row offset and scale fit by legendre
    ...
    for p in (0,1):
        for i in range(entries):        
            ...
            if debug:
                print p, i, profsamples_pi[p,i], 4026 - np.argmax((signal_Rc.T<>0.).any(axis=1)[::-1]),
                print 4026 - np.argmax((usesig_Rc.T<>0.).any(axis=1)[::-1]),            
                print 4026 - np.argmax((fitprof_cR<>0.).any(axis=1)[::-1])
...
#            (prof_ib[i,bsnmax] >= np.clip(prof_ib[i,ok_ib[i]].max(),0,None)))
  ...
  # for fnorm, b0 first guess, use 3 highest ok neighbor bins, fit quadratic                  
    ...
    print okprof_i.sum(), np.where(okprof_i)[0][[0,-1]]
              
    okmax_i = (okprof_i & (ok_ib[i_i,idx0_i-1] & ok_ib[i_i,idx0_i+1]))
    okprof_i = np.copy(okmax_i)

    print "  ",okmax_i.sum(),np.where(okmax_i)[0][[0,-1]]

    fnorm_i = okmax_i.astype(float)*Fnorm_i
  ...
  # cull out width outliers
    ...
    okprofbefore_i = np.copy(okprof_i)      
    okprof_i = legfit_cull(legx_i,width_i,okprof_i,3,IQcull=2.)[1]
    samples_i = (okprof_i[:,None] & ok_iab.any(axis=2)).sum(axis=1)         
  ...
  # fit to Legendre polynomials.  Width is relative to center
    ...
    if debugname:
        ...
        np.savetxt("b0details_i_"+debugname+".txt",     \
            np.vstack((okprof_i,idx0_i,b00_di,pcof0_i,pcof1_i,pcof2_i,b0_i)).T, \
            fmt="%1i %4i %3i %3i %8.3f %9.0f %9.0f %8.3f")        
        np.savetxt("b0width_i_"+debugname+".txt",np.vstack((ok_ib.sum(axis=1),okmax_i.astype(int),    \
            okprofbefore_i.astype(int),okprof_i.astype(int),Fnorm_i,snmax_i,legx_i,b0_i,width_i,b0err_i,widtherr_i)).T,    \
            fmt=4*" %2i"+" %9.1f "+6*" %8.3f ")
