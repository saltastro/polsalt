...
from polutils import rssdtralign, datedfile, datedline, rotate2d
...
# Version 3, returns catalog as a astropy Table with fields named as in SeXtract
    ...
    if (not os.path.exists("out.txt")):
        rsslog.message( "call to SeXtractor failed",logfile)
        exit() 
    ...
    if cull:
        ...
        if debug: rsslog.message( 'SeXtract total: '+str(stars), logfile)
      ...
      # get rid of non-stars based on fwhm compared to median
        ...
        if debug: 
            rsslog.message( 'After duplicate and FWHM cull: '+str(Stars),logfile)
...
def catid(yxcat_dt, yxcand_ds, offsettol=10., errtol=4.,debug=False,logfile='salt.log',name=''):
  ...
  # ids in 3x3 histogram block surrounding histogram max
    ...
    if debug: 
        rsslog.message ((("Catalog entries %3i, Candidates %3i") % (targets,candidates)),logfile)
        rsslog.message ((("  Ids %3i,  unique targets %3i, unique candidates %3i") %   \
                        (ids,uniquetargetids,uniquecandids)),logfile)
        rsslog.message ((("  yx offset (mm):     %8.3f %8.3f") % tuple(offyx_d)),logfile)
  ...
  # get best id in case of multiple id's of same target
    ...
    if ((ids < ok_ts.sum()) and debug): 
        rsslog.message ((("  Ids %3i,  unique candidates %3i") % (ids,uniquecandids)),logfile)
  ...
  # get best id in case of multiple id's of same candidate
    ...
    if ((ids < okt_i.sum()) and debug): 
        rsslog.message ((("  Ids %3i") % ids),logfile)
